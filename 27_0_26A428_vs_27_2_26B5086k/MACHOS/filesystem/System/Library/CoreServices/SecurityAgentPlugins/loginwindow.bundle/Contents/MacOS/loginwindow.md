## loginwindow

> `/System/Library/CoreServices/SecurityAgentPlugins/loginwindow.bundle/Contents/MacOS/loginwindow`

### Sections with Same Size but Changed Content

- `__TEXT.__gcc_except_tab`
- `__DATA_CONST.__const`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_catlist`
- `__DATA_CONST.__objc_superrefs`
- `__DATA_CONST.__objc_intobj`
- `__DATA_CONST.__objc_arraydata`
- `__DATA_CONST.__objc_arrayobj`
- `__DATA_CONST.__got`
- `__DATA.__objc_ivar`
- `__DATA.__objc_data`

```diff

-399.0.0.0.0
-  __TEXT.__text: 0x26df8
+399.2.2.0.0
+  __TEXT.__text: 0x27164
   __TEXT.__auth_stubs: 0xc90
-  __TEXT.__objc_stubs: 0x6300
-  __TEXT.__objc_methlist: 0x21ec
+  __TEXT.__objc_stubs: 0x6320
+  __TEXT.__objc_methlist: 0x2204
   __TEXT.__const: 0x6c
   __TEXT.__gcc_except_tab: 0x4b0
-  __TEXT.__cstring: 0x5d59
+  __TEXT.__cstring: 0x5dc2
   __TEXT.__oslogstring: 0x1c1
-  __TEXT.__objc_methname: 0x6117
-  __TEXT.__objc_classname: 0x294
-  __TEXT.__objc_methtype: 0x1235
+  __TEXT.__objc_methname: 0x6124
+  __TEXT.__objc_classname: 0x2ad
+  __TEXT.__objc_methtype: 0x1224
   __TEXT.__ustring: 0x4
-  __TEXT.__unwind_info: 0xc10
+  __TEXT.__unwind_info: 0xc18
   __DATA_CONST.__const: 0xa98
-  __DATA_CONST.__cfstring: 0x5140
+  __DATA_CONST.__cfstring: 0x51c0
   __DATA_CONST.__objc_classlist: 0x80
   __DATA_CONST.__objc_catlist: 0x10
-  __DATA_CONST.__objc_protolist: 0x68
+  __DATA_CONST.__objc_protolist: 0x70
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_superrefs: 0x68
   __DATA_CONST.__objc_intobj: 0x150

   __DATA_CONST.__objc_arrayobj: 0x18
   __DATA_CONST.__auth_got: 0x658
   __DATA_CONST.__got: 0x3c8
-  __DATA.__objc_const: 0x2000
-  __DATA.__objc_selrefs: 0x1fb8
+  __DATA.__objc_const: 0x2028
+  __DATA.__objc_selrefs: 0x1fc8
   __DATA.__objc_ivar: 0x104
   __DATA.__objc_data: 0x500
-  __DATA.__data: 0x4e8
+  __DATA.__data: 0x548
   __DATA.__crash_info: 0x148
   __DATA.__bss: 0xa8
   __DATA.__common: 0x58

   - /usr/lib/libSystem.B.dylib
   - /usr/lib/libcsfde.dylib
   - /usr/lib/libobjc.A.dylib
-  Functions: 778
+  Functions: 779
   Symbols:   467
-  CStrings:  2066
+  CStrings:  2072
 
CStrings:
+ "AHP_REQ_LOGIN_PIN"
+ "AHP_REQ_TO_ENABLE_PIN"
+ "LUI2UIControllerDelegate"
+ "Not showing Name and Password, style is %d"
+ "TOUCH_ID_OR_PIN"
+ "_activateUntilFrontmost: custom UI is showing, not activating"
+ "activatePlatformSSOWithUserName:options:"
+ "layoutSubtreeIfNeeded"
+ "platformSSOUIVisibilityDidChange"
+ "platformSSOUIVisible"
+ "user list is empty, showing PSSO or name and password"
- "_showNameAndPasswordWithUserName:andPassword:activatePlatformSSO:"
- "showNameAndPasswordWithoutPlatformSSO"
- "user list is empty and PSSO did not activate, showing name and password"
- "user list is empty, re-presented PSSO"
- "v36@0:8@16@24B32"
```

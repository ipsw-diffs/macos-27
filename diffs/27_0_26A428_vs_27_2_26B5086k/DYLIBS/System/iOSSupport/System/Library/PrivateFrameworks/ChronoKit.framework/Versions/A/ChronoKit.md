## ChronoKit

> `/System/iOSSupport/System/Library/PrivateFrameworks/ChronoKit.framework/Versions/A/ChronoKit`

```diff

-749.0.1.0.0
-  __TEXT.__text: 0x2b9c2c
-  __TEXT.__objc_methlist: 0x660
-  __TEXT.__const: 0xffa8
-  __TEXT.__cstring: 0x73db
-  __TEXT.__swift5_typeref: 0x5a20
-  __TEXT.__oslogstring: 0x4843
-  __TEXT.__swift5_capture: 0x9448
-  __TEXT.__swift5_fieldmd: 0x4874
-  __TEXT.__constg_swiftt: 0x83e4
-  __TEXT.__swift5_reflstr: 0x476c
+749.2.4.0.0
+  __TEXT.__text: 0x2c2978
+  __TEXT.__objc_methlist: 0x690
+  __TEXT.__const: 0x100d0
+  __TEXT.__cstring: 0x75b7
+  __TEXT.__swift5_typeref: 0x5a96
+  __TEXT.__oslogstring: 0x4993
+  __TEXT.__swift5_capture: 0x964c
+  __TEXT.__swift5_fieldmd: 0x492c
+  __TEXT.__constg_swiftt: 0x846c
+  __TEXT.__swift5_reflstr: 0x488f
   __TEXT.__swift5_builtin: 0x1a4
   __TEXT.__swift5_assocty: 0xbd8
   __TEXT.__swift5_protos: 0x180
   __TEXT.__swift5_proto: 0x990
-  __TEXT.__swift5_types: 0x538
+  __TEXT.__swift5_types: 0x53c
   __TEXT.__swift5_mpenum: 0x34
   __TEXT.__swift_as_entry: 0xc
   __TEXT.__swift_as_ret: 0x8
   __TEXT.__swift_as_cont: 0x10
-  __TEXT.__unwind_info: 0x4ff8
-  __TEXT.__eh_frame: 0x96c8
+  __TEXT.__unwind_info: 0x5170
+  __TEXT.__eh_frame: 0x9958
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __DATA_CONST.__objc_classlist: 0x378
   __DATA_CONST.__objc_protolist: 0x88
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x820
+  __DATA_CONST.__objc_selrefs: 0x830
   __DATA_CONST.__objc_protorefs: 0x48
-  __DATA_CONST.__got: 0xd50
-  __AUTH_CONST.__const: 0x1bc48
+  __DATA_CONST.__got: 0xd60
+  __AUTH_CONST.__const: 0x1c258
   __AUTH_CONST.__cfstring: 0x120
-  __AUTH_CONST.__objc_const: 0x86c8
-  __AUTH_CONST.__auth_got: 0x1e48
-  __AUTH.__objc_data: 0x1a8
+  __AUTH_CONST.__objc_const: 0x8870
+  __AUTH_CONST.__auth_got: 0x1e98
+  __AUTH.__objc_data: 0x1f8
   __AUTH.__data: 0xa18
   __DATA.__objc_ivar: 0x4
-  __DATA.__data: 0x2280
-  __DATA.__bss: 0x83a0
-  __DATA.__common: 0x498
+  __DATA.__data: 0x2398
+  __DATA.__bss: 0x83b0
+  __DATA.__common: 0x4a0
   __DATA_DIRTY.__objc_data: 0xf80
-  __DATA_DIRTY.__data: 0xac70
+  __DATA_DIRTY.__data: 0xac60
   __DATA_DIRTY.__bss: 0x8680
   __DATA_DIRTY.__common: 0x390
   - /System/Library/Frameworks/ActivityKit.framework/Versions/A/ActivityKit

   - /usr/lib/swift/libswift_DarwinFoundation1.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 12424
-  Symbols:   2588
-  CStrings:  1359
+  Functions: 12559
+  Symbols:   2600
+  CStrings:  1369
 
Symbols:
+ _OBJC_CLASS_$__TtC9ChronoKit30ApplicationAvailabilityMonitor
+ _OBJC_METACLASS_$__TtC9ChronoKit30ApplicationAvailabilityMonitor
+ __DATA__TtC9ChronoKit30ApplicationAvailabilityMonitor
+ __IVARS__TtC9ChronoKit13SmallLRUCache
+ __IVARS__TtC9ChronoKit30ApplicationAvailabilityMonitor
+ __METACLASS_DATA__TtC9ChronoKit30ApplicationAvailabilityMonitor
+ __OBJC_$_INSTANCE_METHODS__TtC9ChronoKit30ApplicationAvailabilityMonitor(ChronoKit)
+ __OBJC_CLASS_PROTOCOLS_$__TtC9ChronoKit30ApplicationAvailabilityMonitor(ChronoKit)
+ _objc_msgSend$enumeratorWithOptions:
+ _objc_msgSend$nextObject
+ _symbolic $s9ChronoKit31ApplicationAvailabilityObserverP
+ _symbolic $s9ChronoKit33ApplicationAvailabilityMonitoringP
+ _symbolic SDyxq_G
+ _symbolic Shy_____y_____GGSg 14ChronoServices15TypedIdentifierV AA0D4TypeO6BundleO9ContainerO
+ _symbolic Shy_____y_____GG_SayyXlGt 14ChronoServices15TypedIdentifierV AA0D4TypeO6BundleO9ContainerO
+ _symbolic Shy_____y_____GGyc 14ChronoServices15TypedIdentifierV AA0D4TypeO6BundleO9ContainerO
+ _symbolic _____ 9ChronoKit13SmallLRUCacheC
+ _symbolic _____ 9ChronoKit30ApplicationAvailabilityMonitorC
+ _symbolic _____Sg 8Dispatch0A4TimeV
+ _symbolic _____Sg 8Dispatch0A8WorkItemC
+ _symbolic _____SgXw 9ChronoKit30ApplicationAvailabilityMonitorC
+ _symbolic _____Sg______t 8Dispatch0A8WorkItemC AA0A4TimeV
+ _symbolic _____XMT 9ChronoKit30ApplicationAvailabilityMonitorC
- _OBJC_CLASS_$__TtC9ChronoKit25ApplicationRemovalMonitor
- _OBJC_METACLASS_$__TtC9ChronoKit25ApplicationRemovalMonitor
- __DATA__TtC9ChronoKit25ApplicationRemovalMonitor
- __IVARS__TtC9ChronoKit25ApplicationRemovalMonitor
- __METACLASS_DATA__TtC9ChronoKit25ApplicationRemovalMonitor
- __OBJC_$_INSTANCE_METHODS__TtC9ChronoKit25ApplicationRemovalMonitor(ChronoKit)
- __OBJC_CLASS_PROTOCOLS_$__TtC9ChronoKit25ApplicationRemovalMonitor(ChronoKit)
- _symbolic $s9ChronoKit26ApplicationRemovalObserverP
- _symbolic $s9ChronoKit28ApplicationRemovalMonitoringP
- _symbolic SaySo18LSApplicationProxyCG
- _symbolic _____ 9ChronoKit25ApplicationRemovalMonitorC
CStrings:
+ " cap — use a cache with O(1) recency for large sizes"
+ " exceeds the "
+ "/AppleInternal/Library/BuildRoots/<BUILDROOT>/Library/Caches/com.apple.xbs/TemporaryDirectory.<TMP>/Sources/Chrono_UI_iosmac/ChronoKit/Extension Management/ApplicationAvailabilityMonitor.swift"
+ "ApplicationAvailabilityMonitor: Starting observation"
+ "ApplicationAvailabilityMonitor: app installs cancelled"
+ "ApplicationAvailabilityMonitor: app installs started"
+ "ApplicationAvailabilityMonitor: applications uninstalled"
+ "ApplicationAvailabilityMonitor: awaiting install %{public}ld - added %{public}s, removed %{public}s"
+ "ApplicationAvailabilityMonitor: ignored %{public}ld of %{public}ld uninstalled proxies - not an LSApplicationProxy, or no bundle identifier"
+ "ApplicationAvailabilityMonitor: invalidating"
+ "ChronoKit/ApplicationAvailabilityMonitor.swift"
+ "ChronoKit/SmallLRUCache.swift"
+ "MacPreviews"
+ "SmallLRUCache capacity must be positive"
+ "SmallLRUCache is O(capacity) per access; "
+ "com.apple.chronod.application-availability-refresh"
- "/AppleInternal"
- "ApplicationRemovalMonitor: Starting observation"
- "ApplicationRemovalMonitor: app installs cancelled"
- "ApplicationRemovalMonitor: applications uninstalled"
- "ApplicationRemovalMonitor: invalidating"
- "ChronoKit/ApplicationRemovalMonitor.swift"
```

## PosterKit

> `/System/iOSSupport/System/Library/PrivateFrameworks/PosterKit.framework/Versions/A/PosterKit`

```diff

-355.0.0.0.0
-  __TEXT.__text: 0x13c57c
-  __TEXT.__objc_methlist: 0x14ae4
-  __TEXT.__const: 0x5664
-  __TEXT.__cstring: 0x93c4
+355.2.4.0.0
+  __TEXT.__text: 0x13c238
+  __TEXT.__objc_methlist: 0x14b04
+  __TEXT.__const: 0x5684
+  __TEXT.__cstring: 0x93d4
   __TEXT.__oslogstring: 0x6f19
   __TEXT.__gcc_except_tab: 0x24ec
   __TEXT.__ustring: 0x1c

   __TEXT.__constg_swiftt: 0x16e4
   __TEXT.__swift5_fieldmd: 0xf3c
   __TEXT.__swift5_builtin: 0xf0
-  __TEXT.__swift5_capture: 0x44c
+  __TEXT.__swift5_capture: 0x45c
   __TEXT.__swift5_proto: 0x1d0
   __TEXT.__swift5_types: 0x148
   __TEXT.__swift5_protos: 0xc
   __TEXT.__swift5_mpenum: 0x118
-  __TEXT.__unwind_info: 0x50a0
+  __TEXT.__unwind_info: 0x5098
   __TEXT.__eh_frame: 0x298
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0

   __DATA_CONST.__objc_catlist2: 0x8
   __DATA_CONST.__objc_protolist: 0x408
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x98b8
+  __DATA_CONST.__objc_selrefs: 0x98d0
   __DATA_CONST.__objc_protorefs: 0xc0
   __DATA_CONST.__objc_superrefs: 0x790
   __DATA_CONST.__objc_arraydata: 0x160
   __DATA_CONST.__got: 0x19d8
-  __AUTH_CONST.__const: 0x3200
+  __AUTH_CONST.__const: 0x3228
   __AUTH_CONST.__cfstring: 0x9920
-  __AUTH_CONST.__objc_const: 0x41a30
+  __AUTH_CONST.__objc_const: 0x41b08
   __AUTH_CONST.__objc_intobj: 0x5b8
   __AUTH_CONST.__objc_arrayobj: 0xd8
   __AUTH_CONST.__objc_doubleobj: 0x190

   __DATA.__objc_ivar: 0x16d0
   __DATA.__data: 0x4538
   __DATA.__objc_stublist: 0x8
-  __DATA.__bss: 0x3890
+  __DATA.__bss: 0x38a0
   __DATA.__common: 0x90
   __DATA_DIRTY.__objc_data: 0x2750
   __DATA_DIRTY.__data: 0x2b0

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 8866
-  Symbols:   16848
+  Functions: 8868
+  Symbols:   16857
   CStrings:  1851
 
Symbols:
+ -[PRRenderer _adaptiveTimeTopY]
+ -[PRRenderer extendRenderingSessionForReason:timeout:]
+ -[PRRenderingSession initWithReason:timeout:invalidationBlock:]
+ GCC_except_table37
+ GCC_except_table45
+ GCC_except_table71
+ GCC_except_table84
+ _PRRenderingSessionDefaultTimeoutInterval
+ _PRRenderingSessionTimeoutSafetyMargin
+ _PRWidgetSnapshotRenderSessionRecommendedTimeout
+ _PRWidgetSnapshotRenderSessionTimeoutIsSufficient
+ __63-[PRRenderingSession initWithReason:timeout:invalidationBlock:]_block_invoke
+ ___54-[PRRenderer extendRenderingSessionForReason:timeout:]_block_invoke
+ ___63-[PRRenderingSession initWithReason:timeout:invalidationBlock:]_block_invoke
+ _objc_msgSend$extendRenderingSessionForReason:timeout:
+ _objc_msgSend$initWithReason:timeout:invalidationBlock:
+ extendRenderingSessionForReason:timeout:.count
- GCC_except_table43
- GCC_except_table69
- GCC_except_table82
- __55-[PRRenderingSession initWithReason:invalidationBlock:]_block_invoke
- ___46-[PRRenderer extendRenderingSessionForReason:]_block_invoke
- ___55-[PRRenderingSession initWithReason:invalidationBlock:]_block_invoke
- _objc_msgSend$initWithReason:invalidationBlock:
- extendRenderingSessionForReason:.count
```

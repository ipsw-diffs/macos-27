## login

> `/System/Library/PrivateFrameworks/login.framework/Versions/A/login`

```diff

-273.0.0.0.0
-  __TEXT.__text: 0x18744
-  __TEXT.__objc_methlist: 0x1334
-  __TEXT.__const: 0x60
-  __TEXT.__gcc_except_tab: 0xd9c
-  __TEXT.__cstring: 0x3c95
+273.2.1.0.0
+  __TEXT.__text: 0x18cd8
+  __TEXT.__objc_methlist: 0x135c
+  __TEXT.__const: 0x68
+  __TEXT.__gcc_except_tab: 0x1024
+  __TEXT.__cstring: 0x3c9f
   __TEXT.__dlopen_cstrs: 0xf5
-  __TEXT.__unwind_info: 0xa48
+  __TEXT.__unwind_info: 0xaf0
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __DATA_CONST.__objc_catlist: 0x8
   __DATA_CONST.__objc_protolist: 0x78
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0xb68
+  __DATA_CONST.__objc_selrefs: 0xb80
   __DATA_CONST.__objc_protorefs: 0x58
   __DATA_CONST.__objc_superrefs: 0x90
   __DATA_CONST.__objc_arraydata: 0x8
   __DATA_CONST.__got: 0x238
-  __AUTH_CONST.__const: 0x908
-  __AUTH_CONST.__cfstring: 0x2760
-  __AUTH_CONST.__objc_const: 0x46f8
+  __AUTH_CONST.__const: 0x938
+  __AUTH_CONST.__cfstring: 0x2780
+  __AUTH_CONST.__objc_const: 0x4728
   __AUTH_CONST.__objc_arrayobj: 0x18
   __AUTH_CONST.__auth_got: 0x0
   __AUTH.__objc_data: 0x690
-  __DATA.__objc_ivar: 0xc0
+  __DATA.__objc_ivar: 0xc4
   __DATA.__data: 0x5b0
   __DATA.__bss: 0x160
   __DATA.__common: 0x20

   - /System/Library/PrivateFrameworks/login.framework/Versions/A/Frameworks/loginsupport.framework/Versions/A/loginsupport
   - /usr/lib/libSystem.B.dylib
   - /usr/lib/libobjc.A.dylib
-  Functions: 594
-  Symbols:   1540
-  CStrings:  519
+  Functions: 598
+  Symbols:   1550
+  CStrings:  520
 
Symbols:
+ -[LFVolume _updateVolumeName]
+ -[LFVolume cachedVolumeName]
+ -[LFVolume setCachedVolumeName:]
+ GCC_except_table109
+ GCC_except_table117
+ GCC_except_table46
+ GCC_except_table49
+ GCC_except_table74
+ GCC_except_table81
+ GCC_except_table83
+ GCC_except_table92
+ OBJC_IVAR_$_LFVolume._cachedVolumeName
+ ___29-[LFVolume _updateVolumeName]_block_invoke
+ ___block_descriptor_40_e8_32s_e5_v8?0l
+ _objc_msgSend$_updateVolumeName
+ _objc_msgSend$cachedVolumeName
+ _objc_msgSend$setCachedVolumeName:
- GCC_except_table115
- GCC_except_table119
- GCC_except_table67
- GCC_except_table75
- GCC_except_table82
- GCC_except_table91
- GCC_except_table98
CStrings:
+ "<Unknown>"
```

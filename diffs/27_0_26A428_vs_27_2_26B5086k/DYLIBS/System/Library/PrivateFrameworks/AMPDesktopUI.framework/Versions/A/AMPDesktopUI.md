## AMPDesktopUI

> `/System/Library/PrivateFrameworks/AMPDesktopUI.framework/Versions/A/AMPDesktopUI`

```diff

-1.7.0.161.2
-  __TEXT.__text: 0xfb510
+1.7.1.18.1
+  __TEXT.__text: 0xfe7c8
   __TEXT.__init_offsets: 0x4
-  __TEXT.__objc_methlist: 0x10e6c
-  __TEXT.__cstring: 0x992d
-  __TEXT.__gcc_except_tab: 0x5310
-  __TEXT.__const: 0x41a4
+  __TEXT.__objc_methlist: 0x111e4
+  __TEXT.__cstring: 0x9a9d
+  __TEXT.__gcc_except_tab: 0x5364
+  __TEXT.__const: 0x4254
   __TEXT.__ustring: 0x98
-  __TEXT.__oslogstring: 0x3d1
-  __TEXT.__swift5_typeref: 0x1f90
-  __TEXT.__swift5_fieldmd: 0x18b0
-  __TEXT.__constg_swiftt: 0x368c
+  __TEXT.__oslogstring: 0x42a
+  __TEXT.__swift5_typeref: 0x2008
+  __TEXT.__swift5_fieldmd: 0x18ec
+  __TEXT.__constg_swiftt: 0x3760
   __TEXT.__swift5_builtin: 0x1cc
-  __TEXT.__swift5_reflstr: 0x164d
+  __TEXT.__swift5_reflstr: 0x168d
   __TEXT.__swift5_assocty: 0x458
-  __TEXT.__swift5_proto: 0x1f8
-  __TEXT.__swift5_types: 0x224
-  __TEXT.__swift5_capture: 0x554
-  __TEXT.__swift5_protos: 0xc
-  __TEXT.__swift_as_entry: 0x8
+  __TEXT.__swift5_proto: 0x200
+  __TEXT.__swift5_types: 0x22c
+  __TEXT.__swift5_capture: 0x5ac
+  __TEXT.__swift5_protos: 0x10
+  __TEXT.__swift_as_entry: 0xc
   __TEXT.__swift_as_ret: 0x8
-  __TEXT.__swift_as_cont: 0xc
-  __TEXT.__unwind_info: 0x59c8
-  __TEXT.__eh_frame: 0x1e0
+  __TEXT.__swift_as_cont: 0x10
+  __TEXT.__unwind_info: 0x5b30
+  __TEXT.__eh_frame: 0x258
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0x1140
-  __DATA_CONST.__objc_classlist: 0x930
+  __DATA_CONST.__const: 0x1170
+  __DATA_CONST.__objc_classlist: 0x968
   __DATA_CONST.__objc_catlist: 0x1b8
   __DATA_CONST.__objc_protolist: 0x3a0
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x8f58
+  __DATA_CONST.__objc_selrefs: 0x9088
   __DATA_CONST.__objc_protorefs: 0x1e0
-  __DATA_CONST.__objc_superrefs: 0x4b8
-  __DATA_CONST.__objc_arraydata: 0xdc0
-  __DATA_CONST.__got: 0xf40
-  __AUTH_CONST.__const: 0x4158
-  __AUTH_CONST.__cfstring: 0xb500
-  __AUTH_CONST.__objc_const: 0x1a860
-  __AUTH_CONST.__objc_arrayobj: 0xfc0
+  __DATA_CONST.__objc_superrefs: 0x4c0
+  __DATA_CONST.__objc_arraydata: 0xde0
+  __DATA_CONST.__got: 0xf78
+  __AUTH_CONST.__const: 0x4300
+  __AUTH_CONST.__cfstring: 0xb600
+  __AUTH_CONST.__objc_const: 0x1aed0
+  __AUTH_CONST.__objc_arrayobj: 0xfd8
   __AUTH_CONST.__objc_intobj: 0x1b0
   __AUTH_CONST.__objc_dictobj: 0x28
   __AUTH_CONST.__objc_doubleobj: 0x20
-  __AUTH_CONST.__auth_got: 0x10f8
-  __AUTH.__objc_data: 0x9b60
-  __AUTH.__data: 0x1588
-  __DATA.__objc_ivar: 0xab0
-  __DATA.__data: 0x3a78
+  __AUTH_CONST.__auth_got: 0x1140
+  __AUTH.__objc_data: 0x9f88
+  __AUTH.__data: 0x1638
+  __DATA.__objc_ivar: 0xad0
+  __DATA.__data: 0x3b08
   __DATA.__objc_stublist: 0x10
   __DATA.__bss: 0x4040
-  __DATA.__common: 0x90
-  __DATA_DIRTY.__objc_data: 0x1078
-  __DATA_DIRTY.__data: 0x198
+  __DATA.__common: 0x98
+  __DATA_DIRTY.__objc_data: 0x1010
+  __DATA_DIRTY.__data: 0x158
   __DATA_DIRTY.__bss: 0x1f8
   __DATA_DIRTY.__common: 0x10
   - /System/Library/Frameworks/AVFoundation.framework/Versions/A/AVFoundation

   - /System/Library/PrivateFrameworks/CommonUtilities.framework/Versions/A/CommonUtilities
   - /System/Library/PrivateFrameworks/CoreAutoLayout.framework/Versions/A/CoreAutoLayout
   - /System/Library/PrivateFrameworks/CoreSymbolication.framework/Versions/A/CoreSymbolication
+  - /System/Library/PrivateFrameworks/IconServices.framework/Versions/A/IconServices
   - /System/Library/PrivateFrameworks/MediaCoreUI.framework/Versions/A/MediaCoreUI
   - /System/Library/PrivateFrameworks/MusicKitInternal.framework/Versions/A/MusicKitInternal
   - /System/Library/PrivateFrameworks/OnBoardingKit.framework/Versions/A/OnBoardingKit

   - /usr/lib/swift/libswiftModelIO.dylib
   - /usr/lib/swift/libswiftOSLog.dylib
   - /usr/lib/swift/libswiftObjectiveC.dylib
+  - /usr/lib/swift/libswiftObservation.dylib
   - /usr/lib/swift/libswiftQuartzCore.dylib
   - /usr/lib/swift/libswiftQuickLookUI.dylib
   - /usr/lib/swift/libswiftSpatial.dylib

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 8611
-  Symbols:   1053
-  CStrings:  1834
+  Functions: 8769
+  Symbols:   1065
+  CStrings:  1849
 
Symbols:
+ _NSScrollViewDidEndLiveScrollNotification
+ _NSScrollViewWillStartLiveScrollNotification
+ _OBJC_CLASS_$_AMPDynamicScrollingFilterableForwarder
+ _OBJC_CLASS_$_AMPDynamicScrollingViewController
+ _OBJC_CLASS_$_AMPFlippedDocumentView
+ _OBJC_CLASS_$_AMPMessageViewController
+ _OBJC_CLASS_$_ISIcon
+ _OBJC_CLASS_$_NSRelativeDateTimeFormatter
+ _OBJC_METACLASS_$_AMPDynamicScrollingFilterableForwarder
+ _OBJC_METACLASS_$_AMPDynamicScrollingViewController
+ _OBJC_METACLASS_$_AMPFlippedDocumentView
+ _OBJC_METACLASS_$_AMPMessageViewController
+ __os_log_debug_impl
+ __os_log_default
+ _os_variant_allows_internal_security_policies
+ _swift_makeBoxUnique
- _OBJC_CLASS_$__TtC12AMPDesktopUI19ErrorViewController
- _OBJC_CLASS_$__TtC12AMPDesktopUI22AMPFlippedDocumentView
- _OBJC_METACLASS_$__TtC12AMPDesktopUI19ErrorViewController
- _OBJC_METACLASS_$__TtC12AMPDesktopUI22AMPFlippedDocumentView
CStrings:
+ "AMPDesktopUI._TrackDownloadCellBase"
+ "AMPDesktopUI/AMPMessageViewController.swift"
+ "Controller asked to pop from window but no navigation stack found to pop from; ignoring."
+ "TRACK_TABLE_PRERELEASE_TRACK_FORMAT"
+ "TRY_AGAIN_BUTTON_TITLE"
+ "box"
+ "dbContentDescriptor"
+ "descriptionText"
+ "filterBarDisplayTitle"
+ "objectValue.expectedReleaseDate"
+ "releaseDate"
+ "showFilterBarBackground"
+ "showFilterBarOverride"
+ "titleHeaderView"
+ "titleOverride"
+ "titleSubtext1View"
+ "titleSubtext2View"
+ "topMarginToObscuredTitle"
- "AMPDesktopUI._TrackDownloadCell"
- "AppIcon"
- "e.square.fill"
```

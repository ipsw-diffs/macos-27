## PhotoAnalysis

> `/System/Library/PrivateFrameworks/PhotoAnalysis.framework/Versions/A/PhotoAnalysis`

```diff

-911.0.134.0.0
-  __TEXT.__text: 0x268ad0
+916.41.100.0.0
+  __TEXT.__text: 0x268a44
   __TEXT.__objc_methlist: 0x5e74
   __TEXT.__const: 0xd1e0
   __TEXT.__constg_swiftt: 0x42a4

   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0xcf0
+  __DATA_CONST.__const: 0xcf8
   __DATA_CONST.__objc_classlist: 0x6a0
   __DATA_CONST.__objc_catlist: 0x20
   __DATA_CONST.__objc_protolist: 0x1d0

   __DATA_CONST.__objc_protorefs: 0xd0
   __DATA_CONST.__objc_superrefs: 0x218
   __DATA_CONST.__objc_arraydata: 0x100
-  __DATA_CONST.__got: 0x1fe0
+  __DATA_CONST.__got: 0x1fc8
   __AUTH_CONST.__const: 0xda38
   __AUTH_CONST.__cfstring: 0x7de0
   __AUTH_CONST.__objc_const: 0x12478

   __AUTH_CONST.__objc_intobj: 0x7b0
   __AUTH_CONST.__objc_arrayobj: 0x138
   __AUTH_CONST.__objc_floatobj: 0x20
-  __AUTH_CONST.__auth_got: 0x2168
+  __AUTH_CONST.__auth_got: 0x2160
   __AUTH.__objc_data: 0xf88
   __AUTH.__data: 0x23d0
   __DATA.__objc_ivar: 0x50c

   __DATA.__bss: 0x7b70
   __DATA.__common: 0x40
   __DATA_DIRTY.__objc_data: 0x2710
-  __DATA_DIRTY.__data: 0x86f8
+  __DATA_DIRTY.__data: 0x86e8
   __DATA_DIRTY.__bss: 0x65c0
   __DATA_DIRTY.__common: 0x43a
   - /System/Library/Frameworks/Accelerate.framework/Versions/A/Accelerate

   - /System/Library/PrivateFrameworks/CoreNameParser.framework/Versions/A/CoreNameParser
   - /System/Library/PrivateFrameworks/CoreRoutine.framework/Versions/A/CoreRoutine
   - /System/Library/PrivateFrameworks/Espresso.framework/Versions/A/Espresso
-  - /System/Library/PrivateFrameworks/FeatureFlags.framework/Versions/A/FeatureFlags
   - /System/Library/PrivateFrameworks/IntelligencePlatform.framework/Versions/A/IntelligencePlatform
   - /System/Library/PrivateFrameworks/KnowledgeGraphKit.framework/Versions/A/KnowledgeGraphKit
   - /System/Library/PrivateFrameworks/MediaAnalysis.framework/Versions/A/MediaAnalysis

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 9283
-  Symbols:   7973
+  Functions: 9282
+  Symbols:   7975
   CStrings:  2969
 
Symbols:
+ OBJC_IVAR_$_PHAWallpaperSuggestionUpgradeSession._stateQueue
+ _PHAWallpaperSuggestionUpgradeSessionStateQueueKey
+ _dispatch_queue_set_specific
- OBJC_IVAR_$_PHAWallpaperSuggestionUpgradeSession._queue
Functions:
~ -[PHAWallpaperSuggestionUpgradeSession init] : 268 -> 292
~ +[PHAWallpaperSuggestionGenerationUtils isGraphDependentSubtype:] : 280 -> 308
- sub_1cc417450
+ sub_1cd2f0c50
- sub_1cc417da0
```

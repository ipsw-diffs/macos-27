## AppStoreComponents

> `/System/Library/PrivateFrameworks/AppStoreComponents.framework/Versions/A/AppStoreComponents`

```diff

-27.0.47.0.0
-  __TEXT.__text: 0x727a4
-  __TEXT.__objc_methlist: 0x6f7c
+27.1.7.0.0
+  __TEXT.__text: 0x722c4
+  __TEXT.__objc_methlist: 0x6f14
   __TEXT.__const: 0x19f4
-  __TEXT.__cstring: 0x3526
-  __TEXT.__oslogstring: 0x2db4
+  __TEXT.__cstring: 0x34e6
+  __TEXT.__oslogstring: 0x2e0b
   __TEXT.__gcc_except_tab: 0x6c4
   __TEXT.__swift5_typeref: 0x486
   __TEXT.__swift5_capture: 0x1c8

   __TEXT.__swift5_proto: 0x100
   __TEXT.__swift5_types: 0xa8
   __TEXT.__swift5_protos: 0xc
-  __TEXT.__unwind_info: 0x2000
+  __TEXT.__unwind_info: 0x1fe8
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __DATA_CONST.__objc_catlist: 0x50
   __DATA_CONST.__objc_protolist: 0xd8
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x3248
+  __DATA_CONST.__objc_selrefs: 0x3220
   __DATA_CONST.__objc_protorefs: 0x38
   __DATA_CONST.__objc_superrefs: 0x368
   __DATA_CONST.__objc_arraydata: 0x60
-  __DATA_CONST.__got: 0x798
+  __DATA_CONST.__got: 0x790
   __AUTH_CONST.__const: 0x2bc0
-  __AUTH_CONST.__cfstring: 0x4960
-  __AUTH_CONST.__objc_const: 0xd008
+  __AUTH_CONST.__cfstring: 0x4940
+  __AUTH_CONST.__objc_const: 0xcfc0
   __AUTH_CONST.__objc_dictobj: 0x50
   __AUTH_CONST.__objc_doubleobj: 0x30
-  __AUTH_CONST.__auth_got: 0xb98
+  __AUTH_CONST.__auth_got: 0xb90
   __AUTH.__objc_data: 0xb18
   __AUTH.__data: 0x5a0
   __DATA.__objc_ivar: 0x6fc

   - /usr/lib/swift/libswift_Builtin_float.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 3184
-  Symbols:   6341
+  Functions: 3171
+  Symbols:   6323
   CStrings:  898
 
Symbols:
+ +[ASCDefaultLockupTheme preferredLabelAlignmentForSize:compatibleWithTraitCollection:]
+ GCC_except_table48
+ GCC_except_table55
+ _objc_msgSend$preferredLabelAlignmentForSize:compatibleWithTraitCollection:
- +[ASCDefaultLockupTheme preferredLabelAlignmentForSize:]
- -[ASCArtworkView setSemanticContentAttribute:]
- -[ASCLockupContentView setSemanticContentAttribute:]
- -[ASCOfferButton setSemanticContentAttribute:]
- -[ASCUIImageView semanticContentAttribute]
- -[ASCUIImageView setSemanticContentAttribute:]
- -[ASCUILabel semanticContentAttribute]
- -[ASCUILabel setSemanticContentAttribute:]
- -[ASCUIView semanticContentAttribute]
- -[ASCUIView setSemanticContentAttribute:]
- -[NSView(AppStoreComponents) asc_layoutTraitEnvironment]
- GCC_except_table49
- GCC_except_table56
- _OUTLINED_FUNCTION_8
- _OUTLINED_FUNCTION_9
- __OBJC_$_PROP_LIST_NSView_$_AppStoreComponents
- _objc_msgSend$asc_layoutTraitEnvironment
- _objc_msgSend$preferredLabelAlignmentForSize:
- _objc_msgSend$semanticContentAttribute
- _objc_msgSend$setSemanticContentAttribute:
- _objc_msgSend$traitEnvironmentWithTraitCollection:
- _objc_msgSend$userInterfaceLayoutDirection
CStrings:
+ "Current process is not eligible to use %{public}@ lockup view size, keeping %{public}@"
- "Current process is not eligible to use %@ lockup view size"
```

## AccessibilityKit

> `/System/Library/PrivateFrameworks/AccessibilitySupport.framework/Versions/A/Frameworks/AccessibilityKit.framework/Versions/A/AccessibilityKit`

```diff

-453.0.0.0.0
-  __TEXT.__text: 0x50a14
+455.1.0.0.0
+  __TEXT.__text: 0x50a88
   __TEXT.__objc_methlist: 0x4234
   __TEXT.__const: 0x339
   __TEXT.__gcc_except_tab: 0x568

   __TEXT.__ustring: 0x12e
   __TEXT.__dlopen_cstrs: 0x72
   __TEXT.__oslogstring: 0x830
-  __TEXT.__unwind_info: 0x1250
+  __TEXT.__unwind_info: 0x1258
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __DATA_CONST.__objc_catlist: 0x10
   __DATA_CONST.__objc_protolist: 0x70
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x30e8
+  __DATA_CONST.__objc_selrefs: 0x30f0
   __DATA_CONST.__objc_superrefs: 0x160
   __DATA_CONST.__objc_arraydata: 0xf8
   __DATA_CONST.__got: 0x768

   - /usr/lib/libSystem.B.dylib
   - /usr/lib/libobjc.A.dylib
   Functions: 2014
-  Symbols:   4595
+  Symbols:   4596
   CStrings:  521
 
Symbols:
+ -[AXKKeyboardFocusTracker _setOriginalSharedElement:]
+ GCC_except_table117
+ GCC_except_table123
+ _objc_msgSend$_setOriginalSharedElement:
+ _objc_msgSend$accessibilityLabelOrTitle
- -[AXKKeyboardFocusTracker set_originalSharedElement:]
- GCC_except_table115
- GCC_except_table119
- _objc_msgSend$set_originalSharedElement:
```

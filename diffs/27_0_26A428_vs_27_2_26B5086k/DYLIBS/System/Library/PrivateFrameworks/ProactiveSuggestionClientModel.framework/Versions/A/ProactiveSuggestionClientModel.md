## ProactiveSuggestionClientModel

> `/System/Library/PrivateFrameworks/ProactiveSuggestionClientModel.framework/Versions/A/ProactiveSuggestionClientModel`

```diff

-671.0.1.0.1
-  __TEXT.__text: 0x74524
-  __TEXT.__objc_methlist: 0x76d4
+674.0.1.0.0
+  __TEXT.__text: 0x746ec
+  __TEXT.__objc_methlist: 0x76ec
   __TEXT.__const: 0x190
   __TEXT.__cstring: 0x49fc
   __TEXT.__oslogstring: 0x5528
-  __TEXT.__gcc_except_tab: 0x4fc
-  __TEXT.__unwind_info: 0x1b40
+  __TEXT.__gcc_except_tab: 0x50c
+  __TEXT.__unwind_info: 0x1b50
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0

   __DATA_CONST.__objc_classlist: 0x3e8
   __DATA_CONST.__objc_protolist: 0xd8
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x2bb0
+  __DATA_CONST.__objc_selrefs: 0x2bc0
   __DATA_CONST.__objc_protorefs: 0x38
   __DATA_CONST.__objc_superrefs: 0x390
   __DATA_CONST.__objc_arraydata: 0x198
   __DATA_CONST.__got: 0x548
-  __AUTH_CONST.__const: 0x16d0
+  __AUTH_CONST.__const: 0x1700
   __AUTH_CONST.__cfstring: 0x4c20
   __AUTH_CONST.__objc_const: 0x139d0
   __AUTH_CONST.__objc_intobj: 0x558

   - /System/Library/PrivateFrameworks/ProtocolBuffer.framework/Versions/A/ProtocolBuffer
   - /usr/lib/libSystem.B.dylib
   - /usr/lib/libobjc.A.dylib
-  Functions: 3251
-  Symbols:   6189
+  Functions: 3254
+  Symbols:   6195
   CStrings:  1042
 
Symbols:
+ -[ATXUniversalBiomeUIStream _currentStoreConfig]
+ -[ATXUniversalBiomeUIStream _storeConfigWithGuardedData:]
+ OBJC_IVAR_$_ATXUniversalBiomeUIStreamGuardedData.storeConfig
+ ___48-[ATXUniversalBiomeUIStream _currentStoreConfig]_block_invoke
+ ___block_descriptor_48_e8_32s40r_e46_v16?0"ATXUniversalBiomeUIStreamGuardedData"8l
+ _objc_msgSend$_currentStoreConfig
+ _objc_msgSend$_storeConfigWithGuardedData:
- OBJC_IVAR_$_ATXUniversalBiomeUIStream._storeConfig
```

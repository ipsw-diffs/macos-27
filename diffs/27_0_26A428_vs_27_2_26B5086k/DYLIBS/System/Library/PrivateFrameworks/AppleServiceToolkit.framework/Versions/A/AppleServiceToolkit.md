## AppleServiceToolkit

> `/System/Library/PrivateFrameworks/AppleServiceToolkit.framework/Versions/A/AppleServiceToolkit`

```diff

-234.0.2.0.0
-  __TEXT.__text: 0x2f740
+234.40.3.0.0
+  __TEXT.__text: 0x2f750
   __TEXT.__objc_methlist: 0x38dc
   __TEXT.__const: 0x170
-  __TEXT.__cstring: 0x2ad6
+  __TEXT.__cstring: 0x2aad
   __TEXT.__oslogstring: 0x22c3
-  __TEXT.__gcc_except_tab: 0x10d0
-  __TEXT.__unwind_info: 0xc98
+  __TEXT.__gcc_except_tab: 0x10f8
+  __TEXT.__unwind_info: 0xca0
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0x3e0
+  __DATA_CONST.__const: 0x3d0
   __DATA_CONST.__objc_classlist: 0x200
   __DATA_CONST.__objc_catlist: 0x30
   __DATA_CONST.__objc_protolist: 0xa8

   __DATA_CONST.__objc_arraydata: 0x110
   __DATA_CONST.__got: 0x340
   __AUTH_CONST.__const: 0xb10
-  __AUTH_CONST.__cfstring: 0x2bc0
-  __AUTH_CONST.__objc_const: 0x63a0
+  __AUTH_CONST.__cfstring: 0x2b80
+  __AUTH_CONST.__objc_const: 0x63c0
   __AUTH_CONST.__objc_arrayobj: 0x48
   __AUTH_CONST.__objc_intobj: 0x210
   __AUTH_CONST.__objc_dictobj: 0x50
   __AUTH_CONST.__auth_got: 0x0
   __AUTH.__objc_data: 0x1248
-  __DATA.__objc_ivar: 0x3a4
+  __DATA.__objc_ivar: 0x3a8
   __DATA.__data: 0x800
-  __DATA.__bss: 0x138
+  __DATA.__bss: 0x130
   __DATA_DIRTY.__objc_data: 0x1b8
   __DATA_DIRTY.__bss: 0x38
   - /System/Library/Frameworks/CoreFoundation.framework/Versions/A/CoreFoundation

   - /usr/lib/libobjc.A.dylib
   - /usr/lib/libz.1.dylib
   Functions: 1285
-  Symbols:   3103
-  CStrings:  664
+  Symbols:   3101
+  CStrings:  662
 
Symbols:
+ OBJC_IVAR_$_ASTEnvironment._assetURL
- _ASTAssetURL
- _ASTTimberLorrySessionKey
- _kASTIdentityAliasDictionaryTimberLorryKey
Functions:
~ -[ASTIdentityAlias init] : 232 -> 96
~ -[ASTEnvironment assetURL] : 12 -> 88
~ -[ASTEnvironment setAssetURL:] : 400 -> 464
~ -[ASTEnvironment .cxx_destruct] : 80 -> 92
CStrings:
- "TimberLorrySession"
- "isTimberLorryTestSession"
```

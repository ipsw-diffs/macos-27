## studentd

> `/usr/libexec/studentd`

### Sections with Same Size but Changed Content

- `__TEXT.__gcc_except_tab`
- `__TEXT.__swift5_builtin`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift5_mpenum`
- `__DATA_CONST.__objc_catlist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__objc_intobj`

```diff

-143.1.1.0.0
-  __TEXT.__text: 0xd7920
-  __TEXT.__auth_stubs: 0x1b30
-  __TEXT.__objc_stubs: 0x12c00
-  __TEXT.__objc_methlist: 0xb09c
-  __TEXT.__objc_methname: 0x19456
-  __TEXT.__objc_classname: 0x2bda
-  __TEXT.__objc_methtype: 0x3f9b
-  __TEXT.__cstring: 0x6402
-  __TEXT.__const: 0x1a60
-  __TEXT.__oslogstring: 0x74de
+148.40.4.0.0
+  __TEXT.__text: 0xe2720
+  __TEXT.__auth_stubs: 0x1f40
+  __TEXT.__objc_stubs: 0x12e20
+  __TEXT.__objc_methlist: 0xb1b4
+  __TEXT.__objc_methname: 0x19800
+  __TEXT.__objc_classname: 0x2cda
+  __TEXT.__objc_methtype: 0x407b
+  __TEXT.__cstring: 0x6896
+  __TEXT.__const: 0x1ea8
+  __TEXT.__oslogstring: 0x79de
   __TEXT.__gcc_except_tab: 0x7bc
   __TEXT.__ustring: 0x818
-  __TEXT.__swift5_typeref: 0xe26
-  __TEXT.__constg_swiftt: 0x8d0
-  __TEXT.__swift5_reflstr: 0x7a3
-  __TEXT.__swift5_fieldmd: 0x570
+  __TEXT.__swift5_typeref: 0xeea
+  __TEXT.__constg_swiftt: 0xa10
+  __TEXT.__swift5_reflstr: 0x813
+  __TEXT.__swift5_fieldmd: 0x6c4
   __TEXT.__swift5_builtin: 0x3c
   __TEXT.__swift5_assocty: 0x18
-  __TEXT.__swift5_proto: 0x84
-  __TEXT.__swift5_types: 0x68
-  __TEXT.__swift5_capture: 0xa68
-  __TEXT.__swift_as_entry: 0x1bc
-  __TEXT.__swift_as_ret: 0x23c
-  __TEXT.__swift_as_cont: 0x4e4
+  __TEXT.__swift5_proto: 0xb8
+  __TEXT.__swift5_types: 0x84
+  __TEXT.__swift5_capture: 0xb28
+  __TEXT.__swift_as_entry: 0x1dc
+  __TEXT.__swift_as_ret: 0x264
+  __TEXT.__swift_as_cont: 0x520
   __TEXT.__swift5_mpenum: 0x10
-  __TEXT.__unwind_info: 0x5030
-  __TEXT.__eh_frame: 0x5c58
-  __DATA_CONST.__const: 0x4cd0
-  __DATA_CONST.__cfstring: 0x48a0
-  __DATA_CONST.__objc_classlist: 0x930
+  __TEXT.__unwind_info: 0x5750
+  __TEXT.__eh_frame: 0x6520
+  __DATA_CONST.__const: 0x5058
+  __DATA_CONST.__cfstring: 0x4940
+  __DATA_CONST.__objc_classlist: 0x950
   __DATA_CONST.__objc_catlist: 0x80
-  __DATA_CONST.__objc_protolist: 0x2d8
+  __DATA_CONST.__objc_protolist: 0x2e8
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_protorefs: 0x58
-  __DATA_CONST.__objc_superrefs: 0x8a0
-  __DATA_CONST.__objc_arraydata: 0x900
-  __DATA_CONST.__objc_dictobj: 0x1680
+  __DATA_CONST.__objc_superrefs: 0x8a8
+  __DATA_CONST.__objc_arraydata: 0x960
+  __DATA_CONST.__objc_dictobj: 0x1770
   __DATA_CONST.__objc_intobj: 0xf0
   __DATA_CONST.__objc_doubleobj: 0x50
-  __DATA_CONST.__auth_got: 0xda8
-  __DATA_CONST.__got: 0x13e8
-  __DATA_CONST.__auth_ptr: 0x2a8
-  __DATA.__objc_const: 0x15980
-  __DATA.__objc_selrefs: 0x53e8
-  __DATA.__objc_ivar: 0x9cc
-  __DATA.__objc_data: 0x6208
-  __DATA.__data: 0x2b80
-  __DATA.__bss: 0x1810
-  __DATA.__common: 0x58
+  __DATA_CONST.__auth_got: 0xfb0
+  __DATA_CONST.__got: 0x14a8
+  __DATA_CONST.__auth_ptr: 0x340
+  __DATA.__objc_const: 0x15da0
+  __DATA.__objc_selrefs: 0x5470
+  __DATA.__objc_ivar: 0x9dc
+  __DATA.__objc_data: 0x6368
+  __DATA.__data: 0x2f20
+  __DATA.__bss: 0x1ea0
+  __DATA.__common: 0x78
   - /System/Library/Frameworks/AVFoundation.framework/Versions/A/AVFoundation
   - /System/Library/Frameworks/AVRouting.framework/Versions/A/AVRouting
   - /System/Library/Frameworks/Accounts.framework/Versions/A/Accounts

   - /System/Library/Frameworks/Foundation.framework/Versions/C/Foundation
   - /System/Library/Frameworks/IOKit.framework/Versions/A/IOKit
   - /System/Library/Frameworks/MediaPlayer.framework/Versions/A/MediaPlayer
+  - /System/Library/Frameworks/OpenDirectory.framework/Versions/A/OpenDirectory
   - /System/Library/Frameworks/ScreenSaver.framework/Versions/A/ScreenSaver
   - /System/Library/Frameworks/Security.framework/Versions/A/Security
   - /System/Library/Frameworks/SystemConfiguration.framework/Versions/A/SystemConfiguration

   - /usr/lib/swift/libswift_StringProcessing.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 5338
-  Symbols:   1170
-  CStrings:  5818
+  Functions: 5521
+  Symbols:   1277
+  CStrings:  5876
 
Symbols:
+ _$s10Foundation11JSONDecoderC6decode_4fromxxm_AA4DataVtKSeRzlFTj
+ _$s10Foundation11JSONDecoderCACycfc
+ _$s10Foundation11JSONDecoderCMa
+ _$s10Foundation13__DataStorageC5bytes6length4copy11deallocator6offsetACSvSg_SiSbySv_SitcSgSitcfc
+ _$s10Foundation13__DataStorageC5bytes6lengthACSVSg_Sitcfc
+ _$s10Foundation13__DataStorageC6_bytesSvSgvg
+ _$s10Foundation13__DataStorageC6lengthACSi_tcfc
+ _$s10Foundation13__DataStorageC7_lengthSivg
+ _$s10Foundation13__DataStorageC7_offsetSivg
+ _$s10Foundation13__DataStorageCMa
+ _$s10Foundation15ContiguousBytesMp
+ _$s10Foundation15ContiguousBytesP010withUnsafeC0yqd__qd__SWKXEKlFTj
+ _$s10Foundation3URLV2eeoiySbAC_ACtFZ
+ _$s10Foundation4DataV10LargeSliceV21ensureUniqueReferenceyyF
+ _$s10Foundation4DataV14RangeReferenceCMa
+ _$s10Foundation4DataV15_RepresentationO15replaceSubrange_4with5countySnySiG_SVSgSitF
+ _$s10Foundation4DataV15_RepresentationO6append10contentsOfySW_tF
+ _$s10Foundation4DataV15_RepresentationON
+ _$s16SafariFoundation19SFGuidedBrowsingTabC15NavigationEventV10httpMethodSSSgvg
+ _$s16SafariFoundation19SFGuidedBrowsingTabC15NavigationEventV10statusCodeSiSgvg
+ _$s16SafariFoundation19SFGuidedBrowsingTabC15NavigationEventV10wasBlockedSbvg
+ _$s16SafariFoundation19SFGuidedBrowsingTabC15NavigationEventV9targetURL0B00I0Vvg
+ _$s16SafariFoundation19SFGuidedBrowsingTabC15NavigationEventVMa
+ _$s16SafariFoundation19SFGuidedBrowsingTabC15NavigationEventVMn
+ _$s16SafariFoundation19SFGuidedBrowsingTabC6StatusC10currentURL0B00H0VvgTj
+ _$s16SafariFoundation19SFGuidedBrowsingTabC6StatusC14lastNavigationAC0H5EventVSgvgTj
+ _$s16SafariFoundation19SFGuidedBrowsingTabC6statusAC6StatusCvgTj
+ _$s7ElementSciTl
+ _$s7FailureSciTl
+ _$sSJ10asciiValues5UInt8VSgvg
+ _$sSJ16wholeNumberValueSiSgvg
+ _$sSJ5write2toyxz_ts16TextOutputStreamRzlF
+ _$sSL2geoiySbx_xtFZTj
+ _$sSS10lowercasedSSyF
+ _$sSS5IndexVMn
+ _$sSS5index5afterSS5IndexVAD_tF
+ _$sSS6append10contentsOfyx_tSTRzSJ7ElementRtzlF
+ _$sSS8UTF8ViewV13_foreignIndex5afterSS0D0VAF_tF
+ _$sSS8UTF8ViewV13_foreignIndex_8offsetBySS0D0VAF_SitF
+ _$sSS8UTF8ViewV17_foreignSubscript8positions5UInt8VSS5IndexV_tF
+ _$sSS8UTF8ViewVN
+ _$sSS9UTF16ViewV5index_8offsetBySS5IndexVAF_SitF
+ _$sSSSysMc
+ _$sSSySJSS5IndexVcig
+ _$sSayxGSesSeRzlMc
+ _$sScI7FailureScI_s5ErrorTn
+ _$sScITL
+ _$sScTss5NeverORszABRs_rlE5sleep5until9tolerance5clocky7InstantQyd___8DurationQyd__Sgqd__tYaKs5ClockRd__lFZ
+ _$sScTss5NeverORszABRs_rlE5sleep5until9tolerance5clocky7InstantQyd___8DurationQyd__Sgqd__tYaKs5ClockRd__lFZTu
+ _$sSe4fromxs7Decoder_p_tKcfCTq
+ _$sSeMp
+ _$sSn10FoundationSS5IndexVRszrlE_2inSnyACGSgSo8_NSRangeV_SShtcfC
+ _$sSo8_NSRangeV10FoundationE_2inABx_q_tcSXRzSyR_SS5IndexV5BoundRtzr0_lufC
+ _$sSs5index5afterSS5IndexVAD_tF
+ _$sSsN
+ _$sSsSTsMc
+ _$sSsySJSS5IndexVcig
+ _$ss11_StringGutsV8copyUTF84intoSiSgSrys5UInt8VG_tF
+ _$ss15ContinuousClockV3nowAB7InstantVvgZ
+ _$ss15ContinuousClockV7InstantV3nowADvgZ
+ _$ss15ContinuousClockV7InstantV8advanced2byADs8DurationV_tF
+ _$ss15ContinuousClockV7InstantVMn
+ _$ss15ContinuousClockV7InstantVSLsMc
+ _$ss16PartialRangeFromVMn
+ _$ss16PartialRangeFromVyxGSXsMc
+ _$ss22KeyedDecodingContainerV6decode_6forKeyS2Sm_xtKF
+ _$ss22KeyedDecodingContainerV6decode_6forKeyS2bm_xtKF
+ _$ss22KeyedDecodingContainerV6decode_6forKeyS2im_xtKF
+ _$ss22KeyedDecodingContainerV6decode_6forKeyqd__qd__m_xtKSeRd__lF
+ _$ss22KeyedDecodingContainerVMn
+ _$ss26DefaultStringInterpolationVN
+ _$ss26DefaultStringInterpolationVs16TextOutputStreamsWP
+ _$ss28CustomDebugStringConvertibleMp
+ _$ss28CustomDebugStringConvertibleP16debugDescriptionSSvgTq
+ _$ss7DecoderP9container7keyedBys22KeyedDecodingContainerVyqd__Gqd__m_tKs9CodingKeyRd__lFTj
+ _$ss8DurationV10componentss5Int64V7seconds_AE11attosecondstvg
+ _$ss9CodingKeyMp
+ _$ss9CodingKeyP11stringValueSSvgTq
+ _$ss9CodingKeyP11stringValuexSgSS_tcfCTq
+ _$ss9CodingKeyP8intValueSiSgvgTq
+ _$ss9CodingKeyP8intValuexSgSi_tcfCTq
+ _$ss9CodingKeyPs23CustomStringConvertibleTb
+ _$ss9CodingKeyPs28CustomDebugStringConvertibleTb
+ _$ss9CodingKeyPsE11descriptionSSvg
+ _$ss9CodingKeyPsE16debugDescriptionSSvg
+ _LoadMCXToolsInterface
+ _NSStringFromCRKGuidedBrowsingContentNavigationFilter
+ _OBJC_CLASS_$_CRKReImportEDUIdentityRequest
+ _OBJC_CLASS_$_CRKReImportEDUIdentityResultObject
+ _OBJC_CLASS_$_ODNode
+ _OBJC_CLASS_$_ODQuery
+ _OBJC_CLASS_$_ODSession
+ _free
+ _getuid
+ _kCRKEDUPayloadCertificateUUIDKey
+ _kODAttributeTypeGUID
+ _kODAttributeTypeUniqueID
+ _kODRecordTypeUsers
+ _swift_arrayInitWithTakeBackToFront
+ _swift_arrayInitWithTakeFrontToBack
+ _swift_coroFrameAlloc
+ _swift_cvw_initEnumMetadataSinglePayloadWithLayoutString
+ _swift_cvw_initStructMetadataWithLayoutString
+ _swift_cvw_initWithTake
+ _swift_cvw_singlePayloadEnumGeneric_destructiveInjectEnumTag
+ _swift_cvw_singlePayloadEnumGeneric_getEnumTag
+ _swift_getEnumTagSinglePayloadGeneric
+ _swift_storeEnumTagSinglePayloadGeneric
- _$s10Foundation3URLV14pathComponentsSaySSGvg
CStrings:
+ " tab(s), allowsOpenInBrowser = "
+ "%d"
+ "%lu profiles contain an EDU payload"
+ "%{public}@: Identity was re-imported: %{public}@. Reason: %{public}@"
+ "%{public}@: New identity persistent ID after re-import: %{public}@. Are persistent IDs different: %{public}@"
+ "%{public}@: No profile configuration; nothing to do"
+ "%{public}@: Profile configuration does not reference an identity payload; nothing to do"
+ "%{public}@: Re-importing the instructor EDU payload identity using MCX. Payload certificate UUID: %{public}@, EDU payload profile identifier: %{public}@, user GUID: %{public}@"
+ "@\"<STUMCXToolsPrimitives>\""
+ "@\"<STUMCXToolsPrimitives>\"16@0:8"
+ "@\"<STUOpenDirectoryPrimitives>\""
+ "@\"<STUOpenDirectoryPrimitives>\"16@0:8"
+ "@\"NSData\"48@0:8@\"NSString\"16@\"NSString\"24@\"NSString\"32^@40"
+ "@\"NSString\"24@0:8^@16"
+ "EDUPayloadProfileIdentifier"
+ "Failed to clamp navigation filter after navigation settled: %{public}@"
+ "Failed to decode site rules: %{public}@"
+ "Failed to generate entire domain regex for %{private}s: %{public}@"
+ "Failed to generate page-only regex for %{private}s: %{public}@"
+ "Failed to generate subpaths regex for %{private}s: %{public}@"
+ "Failed to update existing SFGuidedBrowsingTab with CRKGuidedBrowsingTab %{public}s (%{private}s): %{public}@"
+ "Guided Browsing navigation event: targetURL=%{private}s, statusCode=%{public}s, wasBlocked=%{bool,public}d, httpMethod=%{public}s"
+ "Ignoring site rules at version %{public}ld; this build understands version %{public}ld"
+ "MCXToolsPrimitives"
+ "Navigation settled at %{private}s; clamping navigation filter"
+ "Navigation still active after %{public}llds; clamping on the tab's current URL anyway"
+ "No profile configuration found"
+ "Non-additive site rule %{public}s produced a pattern that rejects the URL it came from; skipping it"
+ "Profile configuration does not contain an identity"
+ "STUConcreteMCXToolsPrimitives_macOS"
+ "STUConcreteOpenDirectoryPrimitives_macOS"
+ "STUMCXToolsPrimitives"
+ "STUOpenDirectoryPrimitives"
+ "STUReImportEDUIdentityOperation"
+ "Site rule %{public}s has an invalid search pattern; skipping it"
+ "T@\"<STUMCXToolsPrimitives>\",R,N"
+ "T@\"<STUMCXToolsPrimitives>\",R,N,V_MCXToolsPrimitives"
+ "T@\"<STUOpenDirectoryPrimitives>\",R,N"
+ "T@\"<STUOpenDirectoryPrimitives>\",R,N,V_openDirectoryPrimitives"
+ "T@\"NSString\",C,N,V_EDUPayloadProfileIdentifier"
+ "TB,N,GisEDUPayloadInstalled,V_EDUPayloadInstalled"
+ "Updating to guided browsing configuration: %{public}s %{private}s"
+ "Using %{public}s navigation filter %{private}s for tab %{public}s"
+ "_EDUPayloadInstalled"
+ "_EDUPayloadProfileIdentifier"
+ "_MCXToolsPrimitives"
+ "_TtCC8studentd24STUGuidedBrowsingSessionP33_8D23AAF5BD83C0A3063E4B76B4B487BD14SettleActivity"
+ "_openDirectoryPrimitives"
+ "defaultSession"
+ "endWithWasReImported:reason:"
+ "firstMatchInString:options:range:"
+ "forceReimportCertificatePayload:inProfile:forUserGUID:error:"
+ "initWithPattern:options:error:"
+ "lastEvent"
+ "nodeWithSession:type:error:"
+ "numberOfRanges"
+ "oldPersistentID"
+ "openDirectoryPrimitives"
+ "profileIdentifier"
+ "queryWithNode:forRecordTypes:attribute:matchType:queryValues:returnAttributes:maximumResults:error:"
+ "rangeAtIndex:"
+ "resultsAllowingPartial:error:"
+ "setEDUPayloadInstalled:"
+ "setEDUPayloadProfileIdentifier:"
+ "setWasIdentityReImported:"
+ "tabStatesByCRKIdentifier"
+ "updateProfileConfiguration:withNewIdentityPersistentID:"
+ "userGUID"
+ "userGUIDWithError:"
+ "v28@0:8B16@20"
+ "valuesForAttribute:error:"
+ "{\n    \"version\": 1,\n    \"rules\": [\n        {\n            \"identifier\": \"google-forms-respond\",\n            \"additive\": true,\n            \"search\": \"(?i:https://docs\\\\.google\\\\.com\\\\.?(?::\\\\d+)?)/forms/(?:u/\\\\d+/)?d/e/([^/?#]+)([/?#].*)?\",\n            \"replace\": \"(?i:https://docs\\\\.google\\\\.com\\\\.?(?::\\\\d+)?)/forms/.*$1.*\"\n        },\n        {\n            \"identifier\": \"google-forms-edit\",\n            \"additive\": true,\n            \"search\": \"(?i:https://docs\\\\.google\\\\.com\\\\.?(?::\\\\d+)?)/forms/(?:u/\\\\d+/)?d/(?!e([/?#]|$))[^/?#]+([/?#].*)?\",\n            \"replace\": \"(?i:https://docs\\\\.google\\\\.com\\\\.?(?::\\\\d+)?)/forms/.*\"\n        },\n        {\n            \"identifier\": \"youtube-watch\",\n            \"additive\": false,\n            \"search\": \"(?i:https://(?:www\\\\.|m\\\\.|music\\\\.)?youtube\\\\.com\\\\.?(?::\\\\d+)?)/watch\\\\?(?:(?!v=)[^&#]*&)*v=([^&#]+)(?:[&#].*)?\",\n            \"replace\": \"(?i:(http|https)://(www\\\\d*\\\\.|m\\\\.|music\\\\.)?youtube\\\\.com\\\\.?(?::\\\\d+)?)/watch\\\\?(?:(?!v=)[^&#]*&)*v=$1(?:[&#].*)?\"\n        }\n    ]\n}"
- "Failed to generate entire domain regex for %{public}s: %{public}@"
- "Failed to generate page-only regex for %{public}s: %{public}@"
- "Failed to generate subpaths regex for %{public}s: %{public}@"
- "Failed to update existing SFGuidedBrowsingTab with CRKGuidedBrowsingTab %{public}@: %{public}@"
- "System"
- "TB,R,N,GisEDUPayloadInstalled"
- "Updating to guided browsing configuration: %{public}s"
- "User"
- "Using navigation filter %{public}s for tab %{public}@"
- "appTags"
- "applicationType"
- "checkIfEDUPayloadInstalledByUserEnrollment"
- "https://docs\\.google\\.com/forms.*"
- "tabTuplesByCRKIdentifier"
```

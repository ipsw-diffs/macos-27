## ExclavesInferenceProvider

> `/System/Library/ExtensionKit/Extensions/ExclavesInferenceProvider.appex/Contents/MacOS/ExclavesInferenceProvider`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methlist`
- `__TEXT.__objc_methtype`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift5_entry`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA.__objc_data`

```diff

-703.0.33.0.0
-  __TEXT.__text: 0x11980
-  __TEXT.__auth_stubs: 0xad0
+714.40.81.501.1
+  __TEXT.__text: 0x181c0
+  __TEXT.__auth_stubs: 0xce0
+  __TEXT.__objc_stubs: 0x40
   __TEXT.__objc_methlist: 0x104
-  __TEXT.__const: 0xe34
-  __TEXT.__objc_classname: 0x192
-  __TEXT.__constg_swiftt: 0x5c8
-  __TEXT.__swift5_typeref: 0x3c8
-  __TEXT.__swift5_fieldmd: 0x524
-  __TEXT.__swift5_builtin: 0x3c
-  __TEXT.__swift5_reflstr: 0x4ba
-  __TEXT.__cstring: 0x781
-  __TEXT.__oslogstring: 0x3e4
-  __TEXT.__swift5_proto: 0x84
-  __TEXT.__swift5_types: 0x64
-  __TEXT.__objc_methname: 0x293
+  __TEXT.__const: 0x1810
+  __TEXT.__objc_classname: 0x275
+  __TEXT.__objc_methname: 0x2c3
   __TEXT.__objc_methtype: 0xb4
-  __TEXT.__swift5_protos: 0x10
+  __TEXT.__constg_swiftt: 0x8f8
+  __TEXT.__swift5_typeref: 0x715
+  __TEXT.__swift5_reflstr: 0x879
+  __TEXT.__swift5_fieldmd: 0x830
+  __TEXT.__swift5_builtin: 0x64
+  __TEXT.__oslogstring: 0x4b4
+  __TEXT.__cstring: 0xb18
+  __TEXT.__swift5_proto: 0x110
+  __TEXT.__swift5_types: 0x90
+  __TEXT.__swift5_protos: 0x30
   __TEXT.__swift5_assocty: 0x30
-  __TEXT.__swift_as_entry: 0x74
-  __TEXT.__swift_as_ret: 0x70
-  __TEXT.__swift_as_cont: 0xa4
-  __TEXT.__swift5_capture: 0x70
+  __TEXT.__swift_as_entry: 0x88
+  __TEXT.__swift_as_ret: 0x88
+  __TEXT.__swift_as_cont: 0xc4
+  __TEXT.__swift5_capture: 0xdc
   __TEXT.__swift5_entry: 0x8
-  __TEXT.__swift5_mpenum: 0x10
-  __TEXT.__unwind_info: 0x748
-  __TEXT.__eh_frame: 0xdb0
-  __DATA_CONST.__const: 0x11e0
-  __DATA_CONST.__objc_classlist: 0x28
+  __TEXT.__swift5_mpenum: 0x20
+  __TEXT.__unwind_info: 0xb20
+  __TEXT.__eh_frame: 0x1778
+  __DATA_CONST.__const: 0x1810
+  __DATA_CONST.__objc_classlist: 0x40
   __DATA_CONST.__objc_protolist: 0x20
   __DATA_CONST.__objc_imageinfo: 0x8
+  __DATA_CONST.__objc_selrefs: 0xb0
   __DATA_CONST.__objc_protorefs: 0x10
-  __DATA_CONST.__auth_got: 0x568
-  __DATA_CONST.__got: 0x228
-  __DATA_CONST.__auth_ptr: 0x220
-  __DATA.__objc_const: 0x5b0
-  __DATA.__objc_selrefs: 0xa0
+  __DATA_CONST.__auth_got: 0x678
+  __DATA_CONST.__got: 0x280
+  __DATA_CONST.__auth_ptr: 0x330
+  __DATA.__objc_const: 0x7d8
   __DATA.__objc_data: 0x98
-  __DATA.__data: 0xa08
-  __DATA.__bss: 0xe00
+  __DATA.__data: 0xc60
+  __DATA.__bss: 0x1c00
   __DATA.__common: 0x230
   - /System/Library/Frameworks/ExtensionFoundation.framework/Versions/A/ExtensionFoundation
   - /System/Library/Frameworks/Foundation.framework/Versions/C/Foundation

   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 584
-  Symbols:   99
-  CStrings:  132
+  - @rpath/AppleInternal/Library/Frameworks/Tightbeam.framework/Versions/A/Tightbeam
+  Functions: 831
+  Symbols:   107
+  CStrings:  158
 
Symbols:
+ _OBJC_CLASS_$_NSFileManager
+ _objc_msgSend
+ _objc_retainAutoreleasedReturnValue
+ _swift_arrayInitWithCopy
+ _swift_arrayInitWithTakeBackToFront
+ _swift_arrayInitWithTakeFrontToBack
+ _swift_initStackObject
+ _swift_setDeallocating
CStrings:
+ "/AppleInternal/usr/local/libexec/exclavetestrelayd"
+ "ExclavesInferenceProvider executing usedSinceLastHysteresisCheck: %s"
+ "ExclavesInferenceProvider/ModelManagerExclaveInternal_swift.swift"
+ "Invalid key value while decoding result type for loadAsset"
+ "Invalid key value while decoding result type for moveAssetToDynamicMode"
+ "Invalid key value while decoding result type for notifyOperationFailure"
+ "Invalid key value while decoding result type for unloadAsset"
+ "Invalid key value while decoding result type for usedSinceLastHysteresisCheck"
+ "_TtC25ExclavesInferenceProvider27ModelManagerExclaveInternal"
+ "_TtCC25ExclavesInferenceProvider27ModelManagerExclaveInternal6Server"
+ "_TtCC25ExclavesInferenceProvider27ModelManagerExclaveInternal7Service"
+ "client"
+ "com.apple.modelmanager.ExclavesInferenceProvider.downcall"
+ "com.apple.modelmanager.exclaves.test.internal"
+ "com.apple.placeholder.imagemodel.base_exclave"
+ "com.apple.placeholder.imagemodel.image_encoder_exclave"
+ "com.apple.placeholder.imagemodel.tokenizer_exclave"
+ "com.apple.unknown3"
+ "com.apple.unknown4.base"
+ "com.apple.unknown5.tokenizer"
+ "com.apple.unknown6.encoder"
+ "connection"
+ "defaultManager"
+ "fileExistsAtPath:"
+ "getOperations returned %s"
+ "illegal variant selector: "
+ "moveAssetToDynamicMode failed: "
+ "moveAssetToDynamicMode: no client, cannot make asset reclaimable"
+ "usedSinceLastHysteresisCheck: no client, reporting not-used"
- "Exclaves not enabled"
- "Exclaves not supported on this platform"
- "ExclavesInferenceProvider/ExclaveProvider.swift"
```

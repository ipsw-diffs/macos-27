## com.apple.MailUI

> `/System/Library/Accessibility/BundlesBase/com.apple.MailUI.axbundle/Versions/A/com.apple.MailUI`

### Sections with Same Size but Changed Content

- `__DATA_CONST.__const`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_superrefs`
- `__DATA.__objc_const`
- `__DATA.__objc_data`

```diff

-334.1.0.0.0
-  __TEXT.__text: 0x27d0
+336.2.0.0.0
+  __TEXT.__text: 0x298c
   __TEXT.__auth_stubs: 0x180
-  __TEXT.__objc_stubs: 0x740
-  __TEXT.__objc_methlist: 0x290
+  __TEXT.__objc_stubs: 0x760
+  __TEXT.__objc_methlist: 0x2a8
   __TEXT.__objc_classname: 0x2c2
-  __TEXT.__cstring: 0x7f7
-  __TEXT.__objc_methname: 0x71c
-  __TEXT.__objc_methtype: 0x5d
+  __TEXT.__cstring: 0x843
+  __TEXT.__objc_methname: 0x757
+  __TEXT.__objc_methtype: 0x68
   __TEXT.__const: 0x18
   __TEXT.__gcc_except_tab: 0x80
-  __TEXT.__unwind_info: 0x160
+  __TEXT.__unwind_info: 0x168
   __DATA_CONST.__const: 0x1d0
-  __DATA_CONST.__cfstring: 0xc20
+  __DATA_CONST.__cfstring: 0xca0
   __DATA_CONST.__objc_classlist: 0x88
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_superrefs: 0x28
   __DATA_CONST.__auth_got: 0xd0
   __DATA_CONST.__got: 0xa0
   __DATA.__objc_const: 0x990
-  __DATA.__objc_selrefs: 0x240
+  __DATA.__objc_selrefs: 0x250
   __DATA.__objc_data: 0x550
   __DATA.__bss: 0x20
   - /System/Library/Frameworks/AppKit.framework/Versions/C/AppKit

   - /usr/lib/libAXSafeCategoryBundle.dylib
   - /usr/lib/libSystem.B.dylib
   - /usr/lib/libobjc.A.dylib
-  Functions: 69
-  Symbols:   275
-  CStrings:  187
+  Functions: 71
+  Symbols:   278
+  CStrings:  193
 
Symbols:
+ -[MUIRichMessageCellBaseAccessibility _accessibilitySortedMessageChildren:]
+ -[MUIRichMessageCellBaseAccessibility accessibilityChildren]
+ ___75-[MUIRichMessageCellBaseAccessibility _accessibilitySortedMessageChildren:]_block_invoke
+ _objc_msgSend$_accessibilitySortedMessageChildren:
- ___77-[MUIRichMessageCellBaseAccessibility accessibilityChildrenInNavigationOrder]_block_invoke
CStrings:
+ "@24@0:8@16"
+ "MailStatusGroup"
+ "RichMessageChildCellView"
+ "_accessibilitySortedMessageChildren:"
+ "flagStatusView"
+ "flagStatusView.cell"
```

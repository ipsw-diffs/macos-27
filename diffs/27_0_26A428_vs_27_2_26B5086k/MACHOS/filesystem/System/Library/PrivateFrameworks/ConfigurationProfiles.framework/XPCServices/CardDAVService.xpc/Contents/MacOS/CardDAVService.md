## CardDAVService

> `/System/Library/PrivateFrameworks/ConfigurationProfiles.framework/XPCServices/CardDAVService.xpc/Contents/MacOS/CardDAVService`

### Sections with Same Size but Changed Content

- `__TEXT.__objc_methlist`
- `__DATA_CONST.__const`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_superrefs`
- `__DATA_CONST.__objc_arraydata`
- `__DATA_CONST.__objc_arrayobj`
- `__DATA_CONST.__objc_intobj`
- `__DATA_CONST.__objc_dictobj`
- `__DATA.__objc_const`
- `__DATA.__objc_data`

```diff

-1911.1.1.0.0
-  __TEXT.__text: 0x8ec4
+1911.40.8.0.0
+  __TEXT.__text: 0x94f8
   __TEXT.__auth_stubs: 0x6c0
-  __TEXT.__objc_stubs: 0xfc0
+  __TEXT.__objc_stubs: 0x1040
   __TEXT.__objc_methlist: 0x5c
-  __TEXT.__gcc_except_tab: 0xcdc
-  __TEXT.__objc_methname: 0xb3b
-  __TEXT.__cstring: 0x1c5a
+  __TEXT.__gcc_except_tab: 0xda0
+  __TEXT.__objc_methname: 0xb96
+  __TEXT.__cstring: 0x1c95
   __TEXT.__objc_classname: 0x20
   __TEXT.__objc_methtype: 0x6e
+  __TEXT.__ustring: 0x24e
   __TEXT.__const: 0x50
   __TEXT.__oslogstring: 0xe
-  __TEXT.__unwind_info: 0x568
+  __TEXT.__unwind_info: 0x590
   __DATA_CONST.__const: 0x470
-  __DATA_CONST.__cfstring: 0x1d60
+  __DATA_CONST.__cfstring: 0x1e40
   __DATA_CONST.__objc_classlist: 0x8
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_superrefs: 0x8

   __DATA_CONST.__auth_got: 0x378
   __DATA_CONST.__got: 0x1e0
   __DATA.__objc_const: 0x90
-  __DATA.__objc_selrefs: 0x420
+  __DATA.__objc_selrefs: 0x440
   __DATA.__objc_data: 0x50
   __DATA.__data: 0x5
   __DATA.__bss: 0x178

   - /usr/lib/libSystem.B.dylib
   - /usr/lib/libc++.1.dylib
   - /usr/lib/libobjc.A.dylib
-  Functions: 196
+  Functions: 201
   Symbols:   182
-  CStrings:  409
+  CStrings:  419
 
CStrings:
+ "<%s:%d NULL format string>"
+ "Cannot install CardDAV account for %@ — a DDM-managed CardDAV account already exists for this host"
+ "CardDAV account creation failed — DDM-managed CardDAV account already exists for host %@"
+ "CreateOrUpdateCardDAVAccount"
+ "Skipping removal of CardDAV account %@ — owned by DDM"
+ "Skipping update of CardDAV account %@ — owned by DDM"
+ "caseInsensitiveCompare:"
+ "initWithFormat:arguments:"
+ "isEqualToNumber:"
+ "managingOwnerIdentifier"
```

## com.apple.Photos.CPLDiagnose

> `/System/Library/PrivateFrameworks/CloudPhotoLibrary.framework/Versions/Current/XPCServices/com.apple.Photos.CPLDiagnose.xpc/Contents/MacOS/com.apple.Photos.CPLDiagnose`

### Sections with Same Size but Changed Content

- `__TEXT.__unwind_info`
- `__DATA_CONST.__const`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__objc_superrefs`
- `__DATA_CONST.__objc_intobj`
- `__DATA_CONST.__objc_arraydata`
- `__DATA_CONST.__objc_arrayobj`
- `__DATA_CONST.__got`
- `__DATA.__objc_data`
- `__DATA.__data`

```diff

-911.0.134.0.0
-  __TEXT.__text: 0x2186c
+916.41.100.0.0
+  __TEXT.__text: 0x2199c
   __TEXT.__auth_stubs: 0xe30
-  __TEXT.__objc_stubs: 0x4460
-  __TEXT.__objc_methlist: 0x1f70
+  __TEXT.__objc_stubs: 0x4440
+  __TEXT.__objc_methlist: 0x1f80
   __TEXT.__const: 0xc0
   __TEXT.__objc_classname: 0x29b
-  __TEXT.__objc_methname: 0x4fd3
+  __TEXT.__objc_methname: 0x4fd8
   __TEXT.__objc_methtype: 0xb10
   __TEXT.__oslogstring: 0x897
-  __TEXT.__cstring: 0x6839
+  __TEXT.__cstring: 0x68cc
   __TEXT.__gcc_except_tab: 0x4cc
   __TEXT.__unwind_info: 0xa30
   __DATA_CONST.__const: 0xea8
-  __DATA_CONST.__cfstring: 0x5c00
+  __DATA_CONST.__cfstring: 0x5ca0
   __DATA_CONST.__objc_classlist: 0x98
   __DATA_CONST.__objc_catlist: 0x10
   __DATA_CONST.__objc_protolist: 0x70

   __DATA_CONST.__auth_got: 0x728
   __DATA_CONST.__got: 0x300
   __DATA_CONST.__auth_ptr: 0x10
-  __DATA.__objc_const: 0x2c30
-  __DATA.__objc_selrefs: 0x1490
-  __DATA.__objc_ivar: 0x228
+  __DATA.__objc_const: 0x2c50
+  __DATA.__objc_selrefs: 0x1488
+  __DATA.__objc_ivar: 0x22c
   __DATA.__objc_data: 0x5f0
   __DATA.__data: 0x558
   __DATA.__bss: 0x100

   - /usr/lib/libobjc.A.dylib
   - /usr/lib/libsqlite3.dylib
   - /usr/lib/libsysdiagnose.dylib
-  Functions: 781
+  Functions: 782
   Symbols:   379
-  CStrings:  2013
+  CStrings:  2018
 
CStrings:
+ "-z"
+ "Getting state capture dictionary"
+ "[-o <outputfile>] [-l <librarypath>] [-s] [-S] [-t] [-d|-D] [-O] [-f <feature>] [-a <annotation>] [-z]%@%@"
+ "_forceTgzArchive"
+ "force the legacy tgz archive format instead of AppleArchive."
+ "o:l:pjtdDu:a:f:LcsSOmPir:nb:z"
+ "photolibraryd-statecapture.txt"
+ "statecapture"
- "[-o <outputfile>] [-l <librarypath>] [-s] [-S] [-t] [-d|-D] [-O] [-f <feature>] [-a <annotation>]%@%@"
- "boolForKey:"
- "o:l:pjtdDu:a:f:LcsSOmPir:nb:"
```

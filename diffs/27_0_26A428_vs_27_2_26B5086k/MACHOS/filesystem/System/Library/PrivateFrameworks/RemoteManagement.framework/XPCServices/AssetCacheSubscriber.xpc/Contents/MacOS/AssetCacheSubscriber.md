## AssetCacheSubscriber

> `/System/Library/PrivateFrameworks/RemoteManagement.framework/XPCServices/AssetCacheSubscriber.xpc/Contents/MacOS/AssetCacheSubscriber`

### Sections with Same Size but Changed Content

- `__TEXT.__swift5_entry`
- `__TEXT.__swift5_fieldmd`
- `__TEXT.__swift5_types`
- `__TEXT.__swift5_capture`
- `__TEXT.__swift5_builtin`
- `__TEXT.__swift5_protos`
- `__TEXT.__swift5_proto`
- `__TEXT.__swift5_mpenum`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift_as_entry`
- `__TEXT.__swift_as_ret`
- `__TEXT.__swift_as_cont`
- `__TEXT.__eh_frame`
- `__DATA_CONST.__const`
- `__DATA_CONST.__objc_classlist`
- `__DATA_CONST.__objc_protolist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA.__data`

```diff

-157.0.0.0.0
-  __TEXT.__text: 0x2f808
-  __TEXT.__auth_stubs: 0xb90
-  __TEXT.__objc_stubs: 0x600
-  __TEXT.__objc_methlist: 0xdf8
-  __TEXT.__const: 0x11b8
-  __TEXT.__cstring: 0x1ae9
-  __TEXT.__swift5_typeref: 0x6f3
+157.40.2.0.0
+  __TEXT.__text: 0x301ec
+  __TEXT.__auth_stubs: 0xbc0
+  __TEXT.__objc_stubs: 0x6a0
+  __TEXT.__objc_methlist: 0xe00
+  __TEXT.__const: 0x11c8
+  __TEXT.__cstring: 0x1b89
+  __TEXT.__swift5_typeref: 0x703
   __TEXT.__swift5_entry: 0x8
   __TEXT.__objc_classname: 0x38c
-  __TEXT.__constg_swiftt: 0xdcc
+  __TEXT.__constg_swiftt: 0xde4
   __TEXT.__swift5_fieldmd: 0xbe0
   __TEXT.__swift5_types: 0x64
-  __TEXT.__objc_methname: 0x2676
+  __TEXT.__objc_methname: 0x2726
   __TEXT.__objc_methtype: 0x70b
   __TEXT.__swift5_capture: 0x3f8
   __TEXT.__swift5_reflstr: 0x12c0

   __TEXT.__swift_as_entry: 0x60
   __TEXT.__swift_as_ret: 0x4c
   __TEXT.__swift_as_cont: 0x38
-  __TEXT.__unwind_info: 0xea0
+  __TEXT.__unwind_info: 0xec0
   __TEXT.__eh_frame: 0x8e0
   __DATA_CONST.__const: 0x2991
   __DATA_CONST.__objc_classlist: 0x58
   __DATA_CONST.__objc_protolist: 0x70
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_protorefs: 0x38
-  __DATA_CONST.__auth_got: 0x5d0
-  __DATA_CONST.__got: 0x150
-  __DATA_CONST.__auth_ptr: 0x3a8
-  __DATA.__objc_const: 0x2260
-  __DATA.__objc_selrefs: 0x768
-  __DATA.__objc_data: 0x11b8
+  __DATA_CONST.__auth_got: 0x5e8
+  __DATA_CONST.__got: 0x170
+  __DATA_CONST.__auth_ptr: 0x3b8
+  __DATA.__objc_const: 0x2268
+  __DATA.__objc_selrefs: 0x798
+  __DATA.__objc_data: 0x11d0
   __DATA.__data: 0xa20
   __DATA.__common: 0xa0
   __DATA.__bss: 0x1380

   - /usr/lib/swift/libswift_Builtin_float.dylib
   - /usr/lib/swift/libswift_Concurrency.dylib
   - /usr/lib/swift/libswiftos.dylib
-  Functions: 1081
-  Symbols:   146
-  CStrings:  575
+  Functions: 1088
+  Symbols:   148
+  CStrings:  583
 
Symbols:
+ _OBJC_CLASS_$_NSByteCountFormatter
+ _OBJC_CLASS_$_NSDateComponentsFormatter
CStrings:
+ "Declarative Status Interval must be a non-negative number of seconds"
+ "Management Reporting Interval must be a non-negative number of seconds"
+ "calendar"
+ "manager:didStartAbsorbingCache:withError:"
+ "setAllowedUnits:"
+ "setUnitsStyle:"
+ "stringFromByteCount:countStyle:"
+ "stringFromTimeInterval:"
```

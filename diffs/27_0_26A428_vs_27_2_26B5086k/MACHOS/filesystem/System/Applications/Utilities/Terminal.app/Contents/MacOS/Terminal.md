## Terminal

> `/System/Applications/Utilities/Terminal.app/Contents/MacOS/Terminal`

### Sections with Same Size but Changed Content

- `__TEXT.__swift5_typeref`
- `__TEXT.__constg_swiftt`
- `__TEXT.__swift5_fieldmd`
- `__TEXT.__swift5_types`
- `__TEXT.__swift5_assocty`
- `__TEXT.__swift5_builtin`
- `__TEXT.__swift5_proto`
- `__TEXT.__swift5_protos`
- `__TEXT.__swift5_capture`
- `__TEXT.__eh_frame`
- `__DATA_CONST.__objc_catlist`
- `__DATA_CONST.__objc_protorefs`
- `__DATA_CONST.__objc_superrefs`
- `__DATA_CONST.__objc_arraydata`
- `__DATA_CONST.__objc_arrayobj`
- `__DATA_CONST.__objc_intobj`
- `__DATA_CONST.__auth_ptr`

```diff

-488.0.0.0.0
-  __TEXT.__text: 0x8aad0
-  __TEXT.__auth_stubs: 0x16e0
-  __TEXT.__objc_stubs: 0x13420
-  __TEXT.__objc_methlist: 0x8c98
-  __TEXT.__const: 0x2a14
-  __TEXT.__gcc_except_tab: 0x6d4
-  __TEXT.__cstring: 0x70c5
+488.4.0.0.0
+  __TEXT.__text: 0x8c6f8
+  __TEXT.__auth_stubs: 0x1720
+  __TEXT.__objc_stubs: 0x137c0
+  __TEXT.__objc_methlist: 0x8e30
+  __TEXT.__const: 0x2a34
+  __TEXT.__gcc_except_tab: 0x740
+  __TEXT.__cstring: 0x71f5
   __TEXT.__ustring: 0x37c
-  __TEXT.__objc_methname: 0x1716b
-  __TEXT.__objc_classname: 0xe17
-  __TEXT.__objc_methtype: 0x416f
-  __TEXT.__oslogstring: 0x18a
+  __TEXT.__objc_methname: 0x176ab
+  __TEXT.__objc_classname: 0xe57
+  __TEXT.__objc_methtype: 0x43bf
+  __TEXT.__oslogstring: 0x1c5
   __TEXT.__swift5_typeref: 0x372
   __TEXT.__constg_swiftt: 0x3b8
   __TEXT.__swift5_reflstr: 0xc3

   __TEXT.__swift5_proto: 0xc8
   __TEXT.__swift5_protos: 0x4
   __TEXT.__swift5_capture: 0x40
-  __TEXT.__unwind_info: 0x2af0
+  __TEXT.__unwind_info: 0x2b88
   __TEXT.__eh_frame: 0x48
-  __DATA_CONST.__const: 0x1f90
-  __DATA_CONST.__cfstring: 0x9160
-  __DATA_CONST.__objc_classlist: 0x358
+  __DATA_CONST.__const: 0x2030
+  __DATA_CONST.__cfstring: 0x9200
+  __DATA_CONST.__objc_classlist: 0x368
   __DATA_CONST.__objc_catlist: 0x68
-  __DATA_CONST.__objc_protolist: 0x140
+  __DATA_CONST.__objc_protolist: 0x148
   __DATA_CONST.__objc_imageinfo: 0x8
   __DATA_CONST.__objc_protorefs: 0x20
   __DATA_CONST.__objc_superrefs: 0x258
   __DATA_CONST.__objc_arraydata: 0x3a8
   __DATA_CONST.__objc_arrayobj: 0xf0
   __DATA_CONST.__objc_intobj: 0x30
-  __DATA_CONST.__auth_got: 0xb80
-  __DATA_CONST.__got: 0xa30
+  __DATA_CONST.__auth_got: 0xba0
+  __DATA_CONST.__got: 0xa48
   __DATA_CONST.__auth_ptr: 0x110
-  __DATA.__objc_const: 0xf9c0
-  __DATA.__objc_selrefs: 0x67e8
-  __DATA.__objc_ivar: 0xad0
-  __DATA.__objc_data: 0x2590
-  __DATA.__data: 0x7570
+  __DATA.__objc_const: 0xfbf8
+  __DATA.__objc_selrefs: 0x6900
+  __DATA.__objc_ivar: 0xae8
+  __DATA.__objc_data: 0x2630
+  __DATA.__data: 0x75d0
   __DATA.__common: 0x50
-  __DATA.__bss: 0x1dc0
+  __DATA.__bss: 0x1df0
   - /System/Library/Frameworks/AppKit.framework/Versions/C/AppKit
   - /System/Library/Frameworks/ApplicationServices.framework/Versions/A/ApplicationServices
   - /System/Library/Frameworks/AudioToolbox.framework/Versions/A/AudioToolbox

   - /usr/lib/swift/libswift_Builtin_float.dylib
   - /usr/lib/swift/libswiftos.dylib
   - /usr/lib/swift/libswiftsimd.dylib
-  Functions: 3084
-  Symbols:   769
-  CStrings:  6208
+  Functions: 3123
+  Symbols:   776
+  CStrings:  6278
 
Symbols:
+ _NSApplicationDidChangeScreenParametersNotification
+ _NSIntersectionRect
+ _NSIsEmptyRect
+ _OBJC_CLASS_$_NSDraggingItem
+ _OBJC_CLASS_$_NSPasteboardItem
+ _dispatch_queue_attr_make_with_qos_class
+ _dispatch_queue_create
CStrings:
+ "@\"NSPanGestureRecognizer\""
+ "@\"NSPressGestureRecognizer\""
+ "@\"NSUserDefaults\""
+ "@24@0:8^{CGRect={CGPoint=dd}{CGSize=dd}}16"
+ "@52@0:8@16@24B32^I36^I44"
+ "B24@0:8@\"NSDraggingSession\"16"
+ "B24@0:8^Q16"
+ "B32@0:8B16B20Q24"
+ "I32@0:8Q16Q24"
+ "NSDraggingSource"
+ "NewTabTarget"
+ "Q32@0:8@\"NSDraggingSession\"16q24"
+ "Q32@0:8@16q24"
+ "Skipped a damaged restored line for profile \"%@\": %@"
+ "TB,V_willJoinTabGroup"
+ "TTAppleScriptTelemetry"
+ "TTApplication.m"
+ "TTBufferInsertObjectsAtOffset(%p, count %lu, offset %lu): offset is out of bounds (count %lu)"
+ "TTDoScriptInNewTabCommand"
+ "_gestureRecognizer:acceptsFirstTouch:event:"
+ "_isTouchDragSession"
+ "_observedDefaults"
+ "_panGestureRecognizer"
+ "_pressGestureRecognizer"
+ "_shouldPreserveSelectionForDrag"
+ "_willJoinTabGroup"
+ "anchorKindForNewTabTarget:"
+ "appleEvent"
+ "beginDraggingSessionWithItems:gesture:source:"
+ "com.apple.Terminal.audio-alert-probe"
+ "containsPoint:"
+ "dataForType:"
+ "decodeRestorationRunsFromCharacterData:runData:isContentsV1:lineSize:lineMark:"
+ "draggingSession:endedAtPoint:operation:"
+ "draggingSession:movedToPoint:"
+ "draggingSession:sourceOperationMaskForDraggingContext:"
+ "draggingSession:willBeginAtPoint:"
+ "fitWindowToCurrentScreen"
+ "frameByFittingFrame:withinVisibleFrame:"
+ "handleDoScriptInNewTabCommand:"
+ "handleFailureInFunction:file:lineNumber:description:"
+ "ignoreModifierKeysForDraggingSession:"
+ "initWithPasteboardWriter:"
+ "isMainThread"
+ "isValidRestorationContentsArray:"
+ "logicalContentSize:fitToMaximum:"
+ "markedTextUnicharOffset:"
+ "maximumLogicalContentSizeForTabController:"
+ "primeAudioAlertCache"
+ "refused a %{public}@ that arrived over Remote Apple Events"
+ "releaseGlobally"
+ "scripting"
+ "selectionDragImageForFrame:"
+ "serializableRunLength:remainingCharacterCount:"
+ "setDraggingFrame:contents:"
+ "setTextSelectionMode:"
+ "setWillJoinTabGroup:"
+ "shouldPositionNewStandaloneWindowWillJoinTabGroup:hasAssignedFrame:numberOfTabs:"
+ "spannedVisibleFrameForFrame:"
+ "spannedVisibleFrameForFrame:screenFrames:visibleFrames:"
+ "the audio-alert cache is main-thread-confined"
+ "trackUsageOnce"
+ "ttCanBeginSelectionDragAtPoint:"
+ "ttPasteboardItemForSelection"
+ "v28@?0i8q12Q20"
+ "v40@0:8@\"NSDraggingSession\"16{CGPoint=dd}24"
+ "v48@0:8@\"NSDraggingSession\"16{CGPoint=dd}24Q40"
+ "void TTArmAudioAlertProbe(void)"
+ "willJoinTabGroup"
+ "{CGRect={CGPoint=dd}{CGSize=dd}}64@0:8{CGRect={CGPoint=dd}{CGSize=dd}}16@48@56"
+ "{CGRect={CGPoint=dd}{CGSize=dd}}80@0:8{CGRect={CGPoint=dd}{CGSize=dd}}16{CGRect={CGPoint=dd}{CGSize=dd}}48"
+ "{CGSize=dd}24@0:8@16"
+ "{CGSize=dd}48@0:8{CGSize=dd}16{CGSize=dd}32"
- "Q20@0:8B16"
- "draggingSourceOperationMaskForLocal:"
- "v20@?0i8q12"
```

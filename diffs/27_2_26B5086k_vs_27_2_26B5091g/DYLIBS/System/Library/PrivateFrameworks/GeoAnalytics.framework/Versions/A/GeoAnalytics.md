## GeoAnalytics

> `/System/Library/PrivateFrameworks/GeoAnalytics.framework/Versions/A/GeoAnalytics`

```diff

-2075.21.6.17.9
-  __TEXT.__text: 0x9ccb8
-  __TEXT.__objc_methlist: 0x2624
-  __TEXT.__const: 0x72c
+2075.24.9.3.1
+  __TEXT.__text: 0x9e1d8
+  __TEXT.__objc_methlist: 0x266c
+  __TEXT.__const: 0x734
   __TEXT.__dlopen_cstrs: 0x54
   __TEXT.__swift5_typeref: 0x4e
   __TEXT.__swift5_capture: 0x24

   __TEXT.__swift5_builtin: 0x14
   __TEXT.__swift5_types: 0x8
   __TEXT.__gcc_except_tab: 0x5ec
-  __TEXT.__cstring: 0xecb3
+  __TEXT.__cstring: 0xece6
   __TEXT.__oslogstring: 0xe6d
-  __TEXT.__unwind_info: 0x1358
+  __TEXT.__unwind_info: 0x1380
   __TEXT.__objc_stubs: 0x0
   __TEXT.__auth_stubs: 0x0
   __TEXT.__objc_classname: 0x0
   __TEXT.__objc_methname: 0x0
   __TEXT.__objc_methtype: 0x0
-  __DATA_CONST.__const: 0x71f0
+  __DATA_CONST.__const: 0x7238
   __DATA_CONST.__objc_classlist: 0x120
   __DATA_CONST.__objc_catlist: 0x8
   __DATA_CONST.__objc_protolist: 0x70
   __DATA_CONST.__objc_imageinfo: 0x8
-  __DATA_CONST.__objc_selrefs: 0x41d8
+  __DATA_CONST.__objc_selrefs: 0x4250
   __DATA_CONST.__objc_protorefs: 0x20
   __DATA_CONST.__objc_superrefs: 0xb8
-  __DATA_CONST.__objc_arraydata: 0xe78
-  __DATA_CONST.__got: 0x6b8
-  __AUTH_CONST.__const: 0x40f8
-  __AUTH_CONST.__cfstring: 0x146c0
+  __DATA_CONST.__objc_arraydata: 0xe88
+  __DATA_CONST.__got: 0x6c0
+  __AUTH_CONST.__const: 0x4118
+  __AUTH_CONST.__cfstring: 0x14700
   __AUTH_CONST.__objc_const: 0x32c8
-  __AUTH_CONST.__objc_intobj: 0x1d88
+  __AUTH_CONST.__objc_intobj: 0x1da0
   __AUTH_CONST.__objc_dictobj: 0x4b0
   __AUTH_CONST.__objc_doubleobj: 0x20
   __AUTH_CONST.__objc_arrayobj: 0x360

   __AUTH.__data: 0x40
   __DATA.__objc_ivar: 0x1f0
   __DATA.__data: 0x550
-  __DATA.__bss: 0x80
+  __DATA.__bss: 0x88
   __DATA.__common: 0x8
   __DATA_DIRTY.__objc_data: 0x910
   __DATA_DIRTY.__data: 0x10
-  __DATA_DIRTY.__bss: 0x1d0
+  __DATA_DIRTY.__bss: 0x1c8
   - /System/Library/Frameworks/CoreFoundation.framework/Versions/A/CoreFoundation
   - /System/Library/Frameworks/Foundation.framework/Versions/C/Foundation
   - /System/Library/PrivateFrameworks/GeoServices.framework/Versions/A/GeoServices

   - /usr/lib/swift/libswiftXPC.dylib
   - /usr/lib/swift/libswift_Builtin_float.dylib
   - /usr/lib/swift/libswiftos.dylib
-  Functions: 1629
-  Symbols:   5162
-  CStrings:  2789
+  Functions: 1640
+  Symbols:   5186
+  CStrings:  2791
 
Symbols:
+ +[GEOAPPortal captureUserAction:target:value:jetVersionId:]
+ +[GEOAPPortal(UserActionCodeGen) captureAccountSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureArpPrivacyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureCarplayUserActionWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureCuratedCollectionsSessionlessUserActionTargetPairRedactedCCStateWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureCuratedCollectionsSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureCuratedCollectionsShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureEnterMapsShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureFamiliarRoutesOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureMapViewEngagementWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureMuninCameraUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureMuninUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureNearbyTransitShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureOfflineSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureOfflineShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) capturePlaceCardRevealWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) capturePredExSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) capturePredExShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) capturePriorityPlacecardActionShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) capturePriorityShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureRapSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureRapShortAndSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureRapUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureSessionlessUserActionWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureStandardShortWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureStartEndNavWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureTransitShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureUGCSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureUGCShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureUserAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:modelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ +[GEOAPPortal(UserActionCodeGen) captureVisualIntelligenceShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
+ -[GEOAPSharedStateData setHasMapSettingsDefaultToFasterRoute:]
+ -[GEOAPSharedStateData setHasMapSettingsMinFasterRouteTimeSavingsS:]
+ -[GEOAPSharedStateData setMapSettingsDefaultToFasterRoute:]
+ -[GEOAPSharedStateData setMapSettingsMinFasterRouteTimeSavingsS:]
+ GCC_except_table1056
+ GCC_except_table1061
+ GCC_except_table1063
+ GCC_except_table1168
+ GCC_except_table1197
+ GCC_except_table1199
+ GCC_except_table1217
+ GCC_except_table1221
+ GCC_except_table1223
+ GCC_except_table1225
+ GCC_except_table1446
+ GCC_except_table1484
+ GCC_except_table565
+ GCC_except_table571
+ GCC_except_table574
+ GCC_except_table577
+ GCC_except_table580
+ GCC_except_table584
+ GCC_except_table587
+ GCC_except_table596
+ GCC_except_table599
+ GCC_except_table602
+ GCC_except_table651
+ GCC_except_table657
+ GCC_except_table658
+ GCC_except_table667
+ _GeoAnalyticsStateConfig_jetMetadata_stateDisabled
+ _GeoAnalyticsStateConfig_jetMetadata_stateDisabled_Metadata
+ _GeoAnalyticsStateConfig_jetMetadata_stateDisabled_Metadata_block_invoke_27
+ _GeoAnalyticsStateConfig_lookAroundSummary_stateDisabled_Metadata_block_invoke_28
+ _GeoAnalyticsStateConfig_lookAroundView_stateDisabled_Metadata_block_invoke_29
+ _GeoAnalyticsStateConfig_mapLaunch_stateDisabled_Metadata_block_invoke_30
+ _GeoAnalyticsStateConfig_mapRestore_stateDisabled_Metadata_block_invoke_31
+ _GeoAnalyticsStateConfig_mapSettingsDetail_stateDisabled_Metadata_block_invoke_33
+ _GeoAnalyticsStateConfig_mapSettings_short_stateDisabled_Metadata_block_invoke_34
+ _GeoAnalyticsStateConfig_mapSettings_stateDisabled_Metadata_block_invoke_32
+ _GeoAnalyticsStateConfig_mapUIShown_stateDisabled_Metadata_block_invoke_36
+ _GeoAnalyticsStateConfig_mapUI_stateDisabled_Metadata_block_invoke_35
+ _GeoAnalyticsStateConfig_mapViewLoc_stateDisabled_Metadata_block_invoke_38
+ _GeoAnalyticsStateConfig_mapView_stateDisabled_Metadata_block_invoke_37
+ _GeoAnalyticsStateConfig_mapsFeatures_stateDisabled_Metadata_block_invoke_39
+ _GeoAnalyticsStateConfig_mapsPlaceIds_stateDisabled_Metadata_block_invoke_40
+ _GeoAnalyticsStateConfig_mapsServer_stateDisabled_Metadata_block_invoke_41
+ _GeoAnalyticsStateConfig_mapsUserSettings_stateDisabled_Metadata_block_invoke_43
+ _GeoAnalyticsStateConfig_mapsUser_stateDisabled_Metadata_block_invoke_42
+ _GeoAnalyticsStateConfig_market_stateDisabled_Metadata_block_invoke_44
+ _GeoAnalyticsStateConfig_muninResource_stateDisabled_Metadata_block_invoke_45
+ _GeoAnalyticsStateConfig_nearbyTransit_stateDisabled_Metadata_block_invoke_46
+ _GeoAnalyticsStateConfig_offlineDownload_stateDisabled_Metadata_block_invoke_48
+ _GeoAnalyticsStateConfig_offline_stateDisabled_Metadata_block_invoke_47
+ _GeoAnalyticsStateConfig_pairedDevice_long_stateDisabled_Metadata_block_invoke_50
+ _GeoAnalyticsStateConfig_pairedDevice_stateDisabled_Metadata_block_invoke_49
+ _GeoAnalyticsStateConfig_photoSubmission_stateDisabled_Metadata_block_invoke_51
+ _GeoAnalyticsStateConfig_placeCardReveal_stateDisabled_Metadata_block_invoke_53
+ _GeoAnalyticsStateConfig_placeCard_rap_stateDisabled_Metadata_block_invoke_54
+ _GeoAnalyticsStateConfig_placeCard_stateDisabled_Metadata_block_invoke_52
+ _GeoAnalyticsStateConfig_placeRequest_stateDisabled_Metadata_block_invoke_55
+ _GeoAnalyticsStateConfig_placeSummaryLayout_stateDisabled_Metadata_block_invoke_56
+ _GeoAnalyticsStateConfig_rap_stateDisabled_Metadata_block_invoke_57
+ _GeoAnalyticsStateConfig_ratingPhotoSubmission_stateDisabled_Metadata_block_invoke_58
+ _GeoAnalyticsStateConfig_ratingSubmission_stateDisabled_Metadata_block_invoke_59
+ _GeoAnalyticsStateConfig_routeSettings_stateDisabled_Metadata_block_invoke_61
+ _GeoAnalyticsStateConfig_route_stateDisabled_Metadata_block_invoke_60
+ _GeoAnalyticsStateConfig_routingWaypoints_stateDisabled_Metadata_block_invoke_62
+ _GeoAnalyticsStateConfig_searchHome_stateDisabled_Metadata_block_invoke_63
+ _GeoAnalyticsStateConfig_searchResults_stateDisabled_Metadata_block_invoke_64
+ _GeoAnalyticsStateConfig_suggestions_stateDisabled_Metadata_block_invoke_65
+ _GeoAnalyticsStateConfig_tapEvent_stateDisabled_Metadata_block_invoke_66
+ _GeoAnalyticsStateConfig_tileSetStatic_stateDisabled_Metadata_block_invoke_68
+ _GeoAnalyticsStateConfig_tileSet_stateDisabled_Metadata_block_invoke_67
+ _GeoAnalyticsStateConfig_transitStep_stateDisabled_Metadata_block_invoke_70
+ _GeoAnalyticsStateConfig_transit_stateDisabled_Metadata_block_invoke_69
+ _GeoAnalyticsStateConfig_ugc_stateDisabled_Metadata_block_invoke_71
+ _GeoAnalyticsStateConfig_widgetProperties_stateDisabled_Metadata_block_invoke_72
+ _GeoAnalyticsStateConfig_widgetTimelineEntries_stateDisabled_Metadata_block_invoke_73
+ _OBJC_CLASS_$_GEOLogMsgStateJetMetadata
+ ___218+[GEOAPPortal(UserActionCodeGen) captureUserAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:modelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___218+[GEOAPPortal(UserActionCodeGen) captureUserAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:modelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke_2
+ ___232+[GEOAPPortal(UserActionCodeGen) captureArpPrivacyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___233+[GEOAPPortal(UserActionCodeGen) captureStartEndNavWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___233+[GEOAPPortal(UserActionCodeGen) captureStartEndNavWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke_2
+ ___235+[GEOAPPortal(UserActionCodeGen) captureStandardShortWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___237+[GEOAPPortal(UserActionCodeGen) capturePlaceCardRevealWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___239+[GEOAPPortal(UserActionCodeGen) captureCarplayUserActionWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___239+[GEOAPPortal(UserActionCodeGen) captureMapViewEngagementWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___240+[GEOAPPortal(UserActionCodeGen) captureFamiliarRoutesOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___243+[GEOAPPortal(UserActionCodeGen) captureSessionlessUserActionWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___244+[GEOAPPortal(UserActionCodeGen) captureRapUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___246+[GEOAPPortal(UserActionCodeGen) captureMuninUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___246+[GEOAPPortal(UserActionCodeGen) capturePriorityShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___250+[GEOAPPortal(UserActionCodeGen) captureUGCShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___252+[GEOAPPortal(UserActionCodeGen) captureMuninCameraUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___253+[GEOAPPortal(UserActionCodeGen) capturePredExShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___254+[GEOAPPortal(UserActionCodeGen) captureOfflineShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___254+[GEOAPPortal(UserActionCodeGen) captureTransitShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___256+[GEOAPPortal(UserActionCodeGen) captureEnterMapsShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___256+[GEOAPPortal(UserActionCodeGen) captureRapSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___256+[GEOAPPortal(UserActionCodeGen) captureUGCSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___256+[GEOAPPortal(UserActionCodeGen) captureVisualIntelligenceShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___259+[GEOAPPortal(UserActionCodeGen) capturePredExSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___260+[GEOAPPortal(UserActionCodeGen) captureAccountSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___260+[GEOAPPortal(UserActionCodeGen) captureNearbyTransitShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___260+[GEOAPPortal(UserActionCodeGen) captureOfflineSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___261+[GEOAPPortal(UserActionCodeGen) capturePriorityPlacecardActionShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___264+[GEOAPPortal(UserActionCodeGen) captureRapShortAndSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___264+[GEOAPPortal(UserActionCodeGen) captureRapShortAndSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke_2
+ ___265+[GEOAPPortal(UserActionCodeGen) captureCuratedCollectionsShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___271+[GEOAPPortal(UserActionCodeGen) captureCuratedCollectionsSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___286+[GEOAPPortal(UserActionCodeGen) captureCuratedCollectionsSessionlessUserActionTargetPairRedactedCCStateWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
+ ___59-[GEOAPSharedStateData setMapSettingsDefaultToFasterRoute:]_block_invoke
+ ___62-[GEOAPSharedStateData setHasMapSettingsDefaultToFasterRoute:]_block_invoke
+ ___65-[GEOAPSharedStateData setMapSettingsMinFasterRouteTimeSavingsS:]_block_invoke
+ ___68-[GEOAPSharedStateData setHasMapSettingsMinFasterRouteTimeSavingsS:]_block_invoke
+ _objc_msgSend$captureAccountSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureArpPrivacyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureCarplayUserActionWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureCuratedCollectionsSessionlessUserActionTargetPairRedactedCCStateWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureCuratedCollectionsSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureCuratedCollectionsShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureEnterMapsShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureFamiliarRoutesOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureMapViewEngagementWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureMuninCameraUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureMuninUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureNearbyTransitShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureOfflineSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureOfflineShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$capturePlaceCardRevealWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$capturePredExSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$capturePredExShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$capturePriorityPlacecardActionShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$capturePriorityShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureRapSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureRapShortAndSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureRapUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureSessionlessUserActionWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureStandardShortWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureStartEndNavWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureTransitShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureUGCSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureUGCShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureUserAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:modelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$captureVisualIntelligenceShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
+ _objc_msgSend$hasMapSettingsDefaultToFasterRoute
+ _objc_msgSend$hasMapSettingsMinFasterRouteTimeSavingsS
+ _objc_msgSend$jetMetadata
+ _objc_msgSend$mapSettingsDefaultToFasterRoute
+ _objc_msgSend$mapSettingsMinFasterRouteTimeSavingsS
+ _objc_msgSend$setBundleVersionId:
+ _objc_msgSend$setDefaultToFasterRoute:
+ _objc_msgSend$setJetMetadata:
+ _objc_msgSend$setMinFasterRouteTimeSavingsS:
+ captureUserAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:modelInfoProviders:additionalStates:completionQueue:completionBlock:.onceToken
+ captureUserAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:modelInfoProviders:additionalStates:completionQueue:completionBlock:.runningInMapsApp
- +[GEOAPPortal(UserActionCodeGen) captureAccountSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureArpPrivacyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureCarplayUserActionWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureCuratedCollectionsSessionlessUserActionTargetPairRedactedCCStateWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureCuratedCollectionsSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureCuratedCollectionsShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureEnterMapsShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureFamiliarRoutesOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureMapViewEngagementWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureMuninCameraUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureMuninUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureNearbyTransitShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureOfflineSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureOfflineShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) capturePlaceCardRevealWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) capturePredExSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) capturePredExShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) capturePriorityPlacecardActionShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) capturePriorityShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureRapSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureRapShortAndSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureRapUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureSessionlessUserActionWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureStandardShortWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureStartEndNavWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureTransitShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureUGCSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureUGCShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- +[GEOAPPortal(UserActionCodeGen) captureVisualIntelligenceShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]
- GCC_except_table1050
- GCC_except_table1058
- GCC_except_table1060
- GCC_except_table1165
- GCC_except_table1194
- GCC_except_table1196
- GCC_except_table1214
- GCC_except_table1218
- GCC_except_table1220
- GCC_except_table1222
- GCC_except_table1435
- GCC_except_table1473
- GCC_except_table563
- GCC_except_table567
- GCC_except_table572
- GCC_except_table575
- GCC_except_table578
- GCC_except_table582
- GCC_except_table583
- GCC_except_table594
- GCC_except_table597
- GCC_except_table600
- GCC_except_table649
- GCC_except_table655
- GCC_except_table656
- GCC_except_table665
- _GeoAnalyticsStateConfig_lookAroundSummary_stateDisabled_Metadata_block_invoke_27
- _GeoAnalyticsStateConfig_lookAroundView_stateDisabled_Metadata_block_invoke_28
- _GeoAnalyticsStateConfig_mapLaunch_stateDisabled_Metadata_block_invoke_29
- _GeoAnalyticsStateConfig_mapRestore_stateDisabled_Metadata_block_invoke_30
- _GeoAnalyticsStateConfig_mapSettingsDetail_stateDisabled_Metadata_block_invoke_32
- _GeoAnalyticsStateConfig_mapSettings_short_stateDisabled_Metadata_block_invoke_33
- _GeoAnalyticsStateConfig_mapSettings_stateDisabled_Metadata_block_invoke_31
- _GeoAnalyticsStateConfig_mapUIShown_stateDisabled_Metadata_block_invoke_35
- _GeoAnalyticsStateConfig_mapUI_stateDisabled_Metadata_block_invoke_34
- _GeoAnalyticsStateConfig_mapViewLoc_stateDisabled_Metadata_block_invoke_37
- _GeoAnalyticsStateConfig_mapView_stateDisabled_Metadata_block_invoke_36
- _GeoAnalyticsStateConfig_mapsFeatures_stateDisabled_Metadata_block_invoke_38
- _GeoAnalyticsStateConfig_mapsPlaceIds_stateDisabled_Metadata_block_invoke_39
- _GeoAnalyticsStateConfig_mapsServer_stateDisabled_Metadata_block_invoke_40
- _GeoAnalyticsStateConfig_mapsUserSettings_stateDisabled_Metadata_block_invoke_42
- _GeoAnalyticsStateConfig_mapsUser_stateDisabled_Metadata_block_invoke_41
- _GeoAnalyticsStateConfig_market_stateDisabled_Metadata_block_invoke_43
- _GeoAnalyticsStateConfig_muninResource_stateDisabled_Metadata_block_invoke_44
- _GeoAnalyticsStateConfig_nearbyTransit_stateDisabled_Metadata_block_invoke_45
- _GeoAnalyticsStateConfig_offlineDownload_stateDisabled_Metadata_block_invoke_47
- _GeoAnalyticsStateConfig_offline_stateDisabled_Metadata_block_invoke_46
- _GeoAnalyticsStateConfig_pairedDevice_long_stateDisabled_Metadata_block_invoke_49
- _GeoAnalyticsStateConfig_pairedDevice_stateDisabled_Metadata_block_invoke_48
- _GeoAnalyticsStateConfig_photoSubmission_stateDisabled_Metadata_block_invoke_50
- _GeoAnalyticsStateConfig_placeCardReveal_stateDisabled_Metadata_block_invoke_52
- _GeoAnalyticsStateConfig_placeCard_rap_stateDisabled_Metadata_block_invoke_53
- _GeoAnalyticsStateConfig_placeCard_stateDisabled_Metadata_block_invoke_51
- _GeoAnalyticsStateConfig_placeRequest_stateDisabled_Metadata_block_invoke_54
- _GeoAnalyticsStateConfig_placeSummaryLayout_stateDisabled_Metadata_block_invoke_55
- _GeoAnalyticsStateConfig_rap_stateDisabled_Metadata_block_invoke_56
- _GeoAnalyticsStateConfig_ratingPhotoSubmission_stateDisabled_Metadata_block_invoke_57
- _GeoAnalyticsStateConfig_ratingSubmission_stateDisabled_Metadata_block_invoke_58
- _GeoAnalyticsStateConfig_routeSettings_stateDisabled_Metadata_block_invoke_60
- _GeoAnalyticsStateConfig_route_stateDisabled_Metadata_block_invoke_59
- _GeoAnalyticsStateConfig_routingWaypoints_stateDisabled_Metadata_block_invoke_61
- _GeoAnalyticsStateConfig_searchHome_stateDisabled_Metadata_block_invoke_62
- _GeoAnalyticsStateConfig_searchResults_stateDisabled_Metadata_block_invoke_63
- _GeoAnalyticsStateConfig_suggestions_stateDisabled_Metadata_block_invoke_64
- _GeoAnalyticsStateConfig_tapEvent_stateDisabled_Metadata_block_invoke_65
- _GeoAnalyticsStateConfig_tileSetStatic_stateDisabled_Metadata_block_invoke_67
- _GeoAnalyticsStateConfig_tileSet_stateDisabled_Metadata_block_invoke_66
- _GeoAnalyticsStateConfig_transitStep_stateDisabled_Metadata_block_invoke_69
- _GeoAnalyticsStateConfig_transit_stateDisabled_Metadata_block_invoke_68
- _GeoAnalyticsStateConfig_ugc_stateDisabled_Metadata_block_invoke_70
- _GeoAnalyticsStateConfig_widgetProperties_stateDisabled_Metadata_block_invoke_71
- _GeoAnalyticsStateConfig_widgetTimelineEntries_stateDisabled_Metadata_block_invoke_72
- ___205+[GEOAPPortal(UserActionCodeGen) captureUserAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:modelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___205+[GEOAPPortal(UserActionCodeGen) captureUserAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:modelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke_2
- ___219+[GEOAPPortal(UserActionCodeGen) captureArpPrivacyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___220+[GEOAPPortal(UserActionCodeGen) captureStartEndNavWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___220+[GEOAPPortal(UserActionCodeGen) captureStartEndNavWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke_2
- ___222+[GEOAPPortal(UserActionCodeGen) captureStandardShortWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___224+[GEOAPPortal(UserActionCodeGen) capturePlaceCardRevealWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___226+[GEOAPPortal(UserActionCodeGen) captureCarplayUserActionWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___226+[GEOAPPortal(UserActionCodeGen) captureMapViewEngagementWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___227+[GEOAPPortal(UserActionCodeGen) captureFamiliarRoutesOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___230+[GEOAPPortal(UserActionCodeGen) captureSessionlessUserActionWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___231+[GEOAPPortal(UserActionCodeGen) captureRapUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___233+[GEOAPPortal(UserActionCodeGen) captureMuninUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___233+[GEOAPPortal(UserActionCodeGen) capturePriorityShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___237+[GEOAPPortal(UserActionCodeGen) captureUGCShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___239+[GEOAPPortal(UserActionCodeGen) captureMuninCameraUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___240+[GEOAPPortal(UserActionCodeGen) capturePredExShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___241+[GEOAPPortal(UserActionCodeGen) captureOfflineShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___241+[GEOAPPortal(UserActionCodeGen) captureTransitShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___243+[GEOAPPortal(UserActionCodeGen) captureEnterMapsShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___243+[GEOAPPortal(UserActionCodeGen) captureRapSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___243+[GEOAPPortal(UserActionCodeGen) captureUGCSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___243+[GEOAPPortal(UserActionCodeGen) captureVisualIntelligenceShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___246+[GEOAPPortal(UserActionCodeGen) capturePredExSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___247+[GEOAPPortal(UserActionCodeGen) captureAccountSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___247+[GEOAPPortal(UserActionCodeGen) captureNearbyTransitShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___247+[GEOAPPortal(UserActionCodeGen) captureOfflineSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___248+[GEOAPPortal(UserActionCodeGen) capturePriorityPlacecardActionShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___251+[GEOAPPortal(UserActionCodeGen) captureRapShortAndSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___251+[GEOAPPortal(UserActionCodeGen) captureRapShortAndSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke_2
- ___252+[GEOAPPortal(UserActionCodeGen) captureCuratedCollectionsShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___258+[GEOAPPortal(UserActionCodeGen) captureCuratedCollectionsSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- ___273+[GEOAPPortal(UserActionCodeGen) captureCuratedCollectionsSessionlessUserActionTargetPairRedactedCCStateWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:]_block_invoke
- _objc_msgSend$captureAccountSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureArpPrivacyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureCarplayUserActionWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureCuratedCollectionsSessionlessUserActionTargetPairRedactedCCStateWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureCuratedCollectionsSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureCuratedCollectionsShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureEnterMapsShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureFamiliarRoutesOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureMapViewEngagementWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureMuninCameraUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureMuninUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureNearbyTransitShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureOfflineSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureOfflineShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$capturePlaceCardRevealWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$capturePredExSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$capturePredExShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$capturePriorityPlacecardActionShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$capturePriorityShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureRapSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureRapShortAndSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureRapUserActionShortOnlyWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureSessionlessUserActionWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureStandardShortWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureStartEndNavWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureTransitShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureUGCSessionlessUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureUGCShortUserActionTargetPairWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- _objc_msgSend$captureVisualIntelligenceShortUserActionsWithAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:dataModelInfoProviders:additionalStates:completionQueue:completionBlock:
- captureUserAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:modelInfoProviders:additionalStates:completionQueue:completionBlock:.onceToken
- captureUserAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:modelInfoProviders:additionalStates:completionQueue:completionBlock:.runningInMapsApp
CStrings:
+ "+[GEOAPPortal(UserActionCodeGen) captureUserAction:target:value:moduleType:moduleMetadata:classification:richProviderId:jetVersionId:sessionSnapshot:modelInfoProviders:additionalStates:completionQueue:completionBlock:]"
+ "JetMetadata"
+ "jetMetadata_stateDisabled"
- "+[GEOAPPortal(UserActionCodeGen) captureUserAction:target:value:moduleType:moduleMetadata:classification:richProviderId:sessionSnapshot:modelInfoProviders:additionalStates:completionQueue:completionBlock:]"
```
